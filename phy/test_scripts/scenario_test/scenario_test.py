...
def inputOptions(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hdpswi:b:m:t:r:",["help","debug","profile","single","waittxstats","sendtoid=","bw=","mcs=","txgain=","rxgain=","txslots=","rxslots=","txchannel=","rxchannel=","stats","rfboost=","numpktstotx=","savetofile","filename="])
    ...
    return debug, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, mcs, txgain, rxgain, tx_slots, rx_slots, tx_channel, rx_channel, debugStats, rfboost, num_of_packets_to_tx, saveStatsToFile, def_filename
...
def sendRxCtrlToPhy(lc, bandwidth, chan, mcs_idx, rx_gain, num_data_slots, seq_number, send_to_id, module, debug):
    ...
    internal.receive.basic_ctrl.timestamp    = 0
...
def receiveStatisticsFromPhyThread(lc, stats_queue, num_of_tx_slots, wait_for_tx_stats, debug, debugStats, saveStatsToFile, def_filename):
    ...
    nof_decoded_pkts = 0 # Total number of received packets.
    nof_wrong_pkts = 0
        ...
        if(getExitFlag() == False and lc.get_low_queue().empty() == False):
            try:
                ...
                if(internal.sendr.result > 0 and internal.sendr.result == PHY_SUCCESS):
                    if(nof_decoded_pkts%5000 == 0): printPhyTxStat(internal)
                    ...
                    if(internal.receiver.result == PHY_SUCCESS):
                       ...
                       if(len(total_rxd_bytes) > 0):
                           nof_decoded_pkts = nof_decoded_pkts + 1 # Increment the number of received packets.
                           ...
                           if(total_rxd_bytes[10] == expected_rxd_byte):
                               ...
                               print("--------> Out of Seq:", out_of_seq_cnt,"- In Seq:", in_seq_cnt, "- Total # pkts:", nof_decoded_pkts, "- % lost packets:", ((out_of_seq_cnt*100)/(in_seq_cnt + out_of_seq_cnt)),  " - Received:", total_rxd_bytes[10], " - Expected:", expected_rxd_byte, "- Host timestamp:",host_timestamp)
                           ...
                           if(nof_decoded_pkts%50 == 0):
                               print("Total number of received packets:",nof_decoded_pkts," - number of in sequence packets:",in_seq_cnt)
                       ...
                       if(debugStats == True):
                           ...
                           avg_cfo = avg_cfo + internal.receiver.stat.rx_stat.cfo
                           if(nof_decoded_pkts%1000 == 0):
                                print("Avg. CQI: %f" % (avg_cqi/avg_counter))
                                print("Avg. RSSI: %f" % (avg_rssi/avg_counter))
                                print("Avg. Noise: %f" % (avg_noise/avg_counter))
                                print("Avg. CFO: %f" % (avg_cfo/avg_counter))

                       if(saveStatsToFile == True):
                           timestamp = internal.receiver.stat.host_timestamp
                           result = internal.receiver.result
                           channel = internal.receiver.stat.ch
                           peak = internal.receiver.stat.rx_stat.peak_value
                           mcs_idx = internal.receiver.stat.mcs
                           cfo = internal.receiver.stat.rx_stat.cfo
                           rssi = internal.receiver.stat.rx_stat.rssi
                           cqi = internal.receiver.stat.rx_stat.cqi
                           noise = internal.receiver.stat.rx_stat.noise
                           sinr = internal.receiver.stat.rx_stat.sinr
                           total_nof_detected = internal.receiver.stat.num_cb_total # Total number of slots synchronized and correctly decoded.
                           nof_detection_errors = internal.receiver.stat.rx_stat.detection_errors
                           nof_decoding_errors = internal.receiver.stat.rx_stat.decoding_errors
                           total_nof_synchronized = internal.receiver.stat.rx_stat.total_packets_synchronized # Total number of synchronized slots, therefore, it will count, false alarms, incorrect decodings, etc.
                           f.write("%d\t%d\t%d\t%d\t%1.5f\t%1.5f\t%1.4f\t%d\t%.10f\t%1.4f\t%d\t%d\t%d\t%d\t%d\n" % (timestamp, result, channel, mcs_idx, peak, cfo, rssi, cqi, noise, sinr, total_nof_detected, nof_detection_errors, nof_decoding_errors, total_nof_synchronized, nof_decoded_pkts))

                       if(nof_decoded_pkts%1000 == 0):
                          print("nof_wrong_pkts: %d - nof_decoded_pkts: %d" % (nof_wrong_pkts, nof_decoded_pkts))

                    else:
                       if(internal.receiver.result == PHY_ERROR):
                          nof_wrong_pkts = nof_wrong_pkts + 1
                          if(saveStatsToFile == True):
                             result = internal.receiver.result
                             timestamp = internal.receiver.stat.host_timestamp
                             channel = internal.receiver.stat.ch
                             ...
                             rssi = internal.receiver.stat.rx_stat.rssi
                             noise = internal.receiver.stat.rx_stat.noise
                             total_nof_detected = internal.receiver.stat.num_cb_total
                             nof_detection_errors = internal.receiver.stat.rx_stat.detection_errors
                             nof_decoding_errors = internal.receiver.stat.rx_stat.decoding_errors
                             total_nof_synchronized = internal.receiver.stat.rx_stat.total_packets_synchronized
                             f.write("%d\t%d\t%d\t%d\t%1.5f\t%1.5f\t%1.4f\t%d\t%.10f\t%1.4f\t%d\t%d\t%d\t%d\t%d\n" % (timestamp, result, channel, 0, peak, cfo, rssi, 0, noise, 0.0, total_nof_detected, nof_detection_errors, nof_decoding_errors, total_nof_synchronized, nof_decoded_pkts))

                          if(nof_wrong_pkts%1000 == 0):
                             print("nof_wrong_pkts: %d - nof_decoded_pkts: %d" % (nof_wrong_pkts, nof_decoded_pkts))
...
if __name__ == '__main__':
    ...
    debug, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, mcs, txgain, rxgain, tx_slots, rx_slots, tx_channel, rx_channel, debugStats, rfboost, num_of_packets_to_tx, saveStatsToFile, def_filename = inputOptions(sys.argv[1:])
    ...
    while(True):
        ...
        time.sleep(num_of_tx_slots*0.001)
