                    ...
                    if(internal.transaction_index == 66):
                       ...
                       if(len(total_rxd_bytes) > 0):
                           ...
                           if(last_rxd_byte != total_rxd_bytes[10]):
                               ...
                               #print("expected byte: %d" % (expected_rxd_byte))
                       ...
                       if(saveStatsToFile == True):
                          f.write("%d\t%1.4f\t%d\t%.10f\t%1.4f\t%d\t%d\t%d\n" % (internal.receiver.stat.mcs,internal.receiver.stat.rx_stat.rssi, internal.receiver.stat.rx_stat.cqi, internal.receiver.stat.rx_stat.noise, internal.receiver.stat.rx_stat.sinr, total_pkt_number, in_seq_cnt, out_of_seq_cnt))
...
def sendEnvironmentUpdateMessage(delay):
    while(getExitFlag() == False):
        os.system("python3 ./process_env_update.py")
        print("Sending enviroment update message\n")
        time.sleep(delay)

def rxParametersThread(lc, mcs_idx, num_of_rx_slots, seq_num, send_to, source_module, debug):
   while(getExitFlag() == False):
      nof_prb = random.randint(0, 3)
      bandwidth = getBWIndex(PHY_PRB_VECTOR[nof_prb])
      rx_channel = random.randint(0, 10)
      rx_gain = random.randint(0, 30)
      phy_id = random.randint(0, 1)
      print("[Rx parameters] nof_prb: %d - rx_channel: %d - rx_gain: %d phy_id: %d" % (PHY_PRB_VECTOR[nof_prb], rx_channel, rx_gain, phy_id))
      # Send RX control information to PHY.
      sendRxCtrlToPhy(lc, bandwidth, rx_channel, mcs_idx, rx_gain, num_of_rx_slots, seq_num, send_to, phy_id, source_module, debug)
      time.sleep(0.5)
...
if __name__ == '__main__':
    ...
    run_rx_params_thread = True
    run_tx_params_thread = True
    run_env_update_thread = True
    tx_random_nof_prb = True
    send_to = send_to_id
    ...
    # Start thread to change Rx parameters.
    if(run_rx_params_thread == True):
        try:
            thread_id = _thread.start_new_thread( rxParametersThread, (lc, mcs_idx, num_of_rx_slots, 66, send_to, source_module, debug, ) )
        except:
            print("Error: unable to start rxParametersThread")
            sys.exit(-1)

        # Give the thread some time.
        time.sleep(1)

    # Start thread to change enviroment parameters.
    if(run_env_update_thread == True):
        try:
            thread_id = _thread.start_new_thread( sendEnvironmentUpdateMessage, (delay, ) )
        except:
            print("Error: unable to start sendEnvironmentUpdateMessage")
            sys.exit(-1)
    ...
    while(getExitFlag() == False):
        ...
        if(run_tx_params_thread == True):

            packet_counter = packet_counter + 1

            if(tx_random_nof_prb == True):
                nof_prb = PHY_PRB_VECTOR[random.randint(0, 3)]
                bandwidth = getBWIndex(nof_prb)
            tx_channel = random.randint(0, 10)
            tx_gain = random.randint(0, 30)
            phy_id = random.randint(0, 1)
            print("[Tx parameters] nof_prb: %d - rx_channel: %d - tx_gain: %d phy_id: %d" % (nof_prb, tx_channel, tx_gain, phy_id))

            # Generate data according to PHY BW and MCS.
            data, length = generateData3(num_of_tx_slots, seq_number-99, bandwidth, mcs_idx) # Generate data according to sequence number.
