...
if __name__ == '__main__':
    ...
    for prb_idx in range(0,4):
        ...
        for mcs_idx in range(0,29,2):
            ...
            tb_size = getTransportBlockSize(bw_idx, mcs_idx)

            streaming_tput = (tb_size*8)/0.001

            for nof_slots_idx in range(0,3):

                nof_slots = NOF_SLOTS_VECTOR[nof_slots_idx]

                dc_percentage = (0.5*100)/(nof_slots+0.5)

                dtx_tput = ((100-dc_percentage)/100)*streaming_tput

                print("%d\t%d\t%d\t%d\t%f\t%f\t%f" % (prb,mcs_idx,tb_size,nof_slots,dc_percentage,streaming_tput,dtx_tput))
