...
def sendRxCtrlToPhy(lc, bandwidth, chan, mcs_idx, rx_gain, num_data_slots, seq_number, send_to_id, module, print_stats):
...
def sendTxCtrlToPhy(lc, chan, bandwidth, mcs_idx, gain_value, slots, sequence_number, send_to_id, module, data, print_stats):
...
def receiveStatisticsFromPhyThread(lc, stats_queue, num_of_tx_slots, wait_for_tx_stats, calculate_tput, min_seq_number_tx, max_seq_number_tx, min_seq_number_rx, max_seq_number_rx, print_stats):
    ...
    if(print_stats == False):
        stats_fig = SenSysDemo.SenSysDemo()
        stats_fig.start()
            ...
            try:
                ...
                if(internal.sendr.result > 0 and internal.sendr.result == PHY_SUCCESS):
                    if(internal.transaction_index >= min_seq_number_tx and internal.transaction_index <= max_seq_number_tx):
                        ...
                        if(tx_stats_cnt%NUMBER_OF_NECESSARY_STATS == 0):
                    ...
                    if(internal.receiver.result == PHY_SUCCESS):
                       ...
                       if(len(total_rxd_bytes) > 0):
                           ...
                           #if(print_stats == False):
                        #       stats_fig.plot_correct_rx_stats(internal.receiver.stat.rx_stat.rssi,internal.receiver.stat.rx_stat.cqi,internal.receiver.stat.rx_stat.noise)
                    ...
                    # Treat PHY_ERROR case.
                        ...
                        #if(print_stats == False):
                        #    stats_fig.plot_wrong_rx_stats(internal.receiver.stat.rx_stat.rssi,internal.receiver.stat.rx_stat.noise)
...
def print_phy_success_rx_stats(correctly_received_pkt_cnt, out_of_seq_cnt, in_seq_cnt, throughput, throughput_cnt, internal):
    ...
    if(correctly_received_pkt_cnt > 0):
        print(" - lost packets: %1.2f %%" % ((out_of_seq_cnt*100)/correctly_received_pkt_cnt), end="")
    if(throughput_cnt > 0):
...
if __name__ == '__main__':
    ...
    print_stats, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, mcs_idx, tx_gain, rx_gain, num_of_tx_slots, num_of_rx_slots, rx_only, tx_only, calculate_tput, delay_after_tx = inputOptions(sys.argv[1:])
    ...
    try:
        thread_id = _thread.start_new_thread( receiveStatisticsFromPhyThread, (lc, stats_queue, num_of_tx_slots, wait_for_tx_stats, calculate_tput, min_seq_number_tx, max_seq_number_tx, min_seq_number_rx, max_seq_number_rx, print_stats, ) )
    except:
    ...
    sendRxCtrlToPhy(lc, bandwidth, channel, mcs_idx, rx_gain, num_of_rx_slots, 66, send_to_id, source_module, print_stats)
    ...
    if(rx_only == True):
        ...
        while(True):
            ...
            sendTxCtrlToPhy(lc, channel, bandwidth, mcs_idx, tx_gain, num_of_tx_slots, seq_number, send_to_id, source_module, data, print_stats)
