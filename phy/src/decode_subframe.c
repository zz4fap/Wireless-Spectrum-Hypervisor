...
int main() {
  ...
  for(uint32_t file_cnt = 0; file_cnt < NUMBER_OF_FILES; file_cnt++) {
    ...
    //sprintf(filename, "/home/zz4fap/work/backup_plan/scatter/build/check_subframe_discontinuity_sf_10_cnt_%d.dat",file_cnt);
    ...
    printf("[APP] peak_value: %1.2f - peak_position: %d\n", sfind.peak_value, peak_position);

    // Apply CFO to the signal.
    srslte_cfo_correct_finer(&cfocorr, subframe_buffer, subframe_buffer, 0.0/384.0);

    float mean_cfo2 = srslte_pss_synch_cfo_compute(&pss, &subframe_buffer[peak_position - srslte_symbol_sz(cell_ue.nof_prb)]);
    printf("----> PSS-based CFO estimation: %1.4f\n", mean_cfo2*15000.0);

    //int peak_pos;
    //float peak_value;
    //int cfo_i = cfo_i_estimate(&sfind, subframe_buffer, 0, &peak_pos, &peak_value);
    //printf("----> cfo_i: %d - peak_pos: %d - peak_value: %f\n", cfo_i, peak_pos, peak_value);

    float cfo_est = cfo_estimate_cp(&sfind, subframe_buffer);
