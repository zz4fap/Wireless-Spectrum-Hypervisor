...
s_14M_6RB = modulated_signal;

% --------------1.4 MHz LTE tx: 1 CP + 1 OFDM symbol----------------------
...
% figure; 
% plot(abs(s_20M_100RB)); title('multiplexed two 6RB in 100RB for 20M tx'); 
% grid on;
...
fdee_figure = figure('rend','painters','pos',[10 10 900 800]);
title('Time domain imaginary part of 1.4M LTE signal');

subplot(2,1,1)
plot(real(ofdm_symbol_with_cp_14M),'k', 'LineWidth', 1);
title('Real part')
grid on; 
hold on;
plot(real(rx_p6MHz),'bs', 'LineWidth', 1);
plot(real(rx_n6MHz),'r.', 'LineWidth', 1);
xlabel('Sample number')
ylabel('Amplitude')
xlim([1 length(ofdm_symbol_with_cp_14M)])
%legend('1.4M tx by 1.92Msps LTE tx', '1.4M rx0 from 20M LTE tx', '1.4M rx1 from 20M LTE tx');
ld = legend('PHY', 'vPHY #1', 'vPHY #2');
ld.FontSize = 11;



subplot(2,1,2)
plot(imag(ofdm_symbol_with_cp_14M),'k', 'LineWidth', 1);
title('Imaginary part')
grid on; 
hold on;
plot(imag(rx_p6MHz),'bs', 'LineWidth', 1);
plot(imag(rx_n6MHz),'r.', 'LineWidth', 1);
xlabel('Sample number')
ylabel('Amplitude')
xlim([1 length(ofdm_symbol_with_cp_14M)])
ld = legend('PHY', 'vPHY #1', 'vPHY #2');
...
error_s1 = sum(abs(ofdm_symbol_with_cp_14M - rx_n6MHz).^2)/length(rx_n6MHz)


error_s2 = sum(abs(ofdm_symbol_with_cp_14M - rx_p6MHz).^2)/length(rx_p6MHz)
