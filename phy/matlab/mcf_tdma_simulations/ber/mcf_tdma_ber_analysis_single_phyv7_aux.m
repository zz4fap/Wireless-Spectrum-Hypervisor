...
sigPowerdB = -23.5833;%-45.166; % Average signal power in dB for a single vPHY transmitting.
...
cuttoff_freq = ((sampling_rate_20MHz/2)/12)/(sampling_rate_20MHz/2);
...
numIter_vector = 1e5*ones(1, length(SNR));
...
for snr_idx=1:1:length(SNR)
    ...
    for iter=1:1:numIter
        ...
        for ofdm_symbol_idx = 2:1:2
            ...
            if(ofdm_symbol_idx > 1)
                sigPower = sum(abs(tx_ofdm_symbol_with_cp_23dot04MHz(:)).^2)/length(tx_ofdm_symbol_with_cp_23dot04MHz(:)); % linear
                sigPowerdB_aux = 10*log10(sigPower);
                
                sigPowerdB_mean = sigPowerdB_mean + sigPowerdB_aux;
                sigPowerdB_cnt = sigPowerdB_cnt + 1;
            end
        ...
        if(mod(iter,100) == 0)
            fprintf(1, 'Single-PHY - SNR: %1.2f [dB] - iter: %d\n', SNR(snr_idx), iter);
        end
        
    end
    
    sigPowerdB_mean = sigPowerdB_mean/sigPowerdB_cnt;
    
    fprintf(1,'SNR: %1.2f [dB] - numIter: %d - sigPowerdB_mean: %1.4f\n', SNR(snr_idx), numIter, sigPowerdB_mean);
    
end

%EbNodB =  SNR - ( 10*log10(modOrd) + 10*log10((nfft_1dot4MHz)/nfft_23dot04MHz) );
%EbNodB =  SNR - ( 10*log10(modOrd)  );
...
if(0)
    ...
    fileName = sprintf('mcf_tdma_ber_analysis_single_phy_v7_%s.mat', timeStamp);
    save(fileName);
end
