...
channelizer = dsp.Channelizer(numOfvPHYsIn20MHz, 'StopbandAttenuation', 10, 'NumTapsPerBand', 12);
...
numIter_vector = [10000*ones(1, 11) 1000000*ones(1, 5)];
    ...
    for iter=1:1:numIter
        ...
        for ofdm_symbol_idx = 1:1:2
            ...
            for vphy_idx = 0:1:numOfvPHYs-1
                ...
                % Adding CP.
                tx_ofdm_symbol_with_cp_23dot04MHz = 12*[ofdm_symbol_23dot04MHz((end-(len_cp_20MHz-1)):end); ofdm_symbol_23dot04MHz];
            ...
            %figure;
            %plot(0:1:nfft_23dot04MHz-1, abs(fft(noisy_tx_ofdm_symbol_with_cp_23dot04MHz(len_cp_20MHz+1:end), nfft_23dot04MHz)));
            %grid on;
            ...
            t_idx = (-len_cp_20MHz:(length(noisy_tx_ofdm_symbol_with_cp_23dot04MHz)-len_cp_20MHz-1)).';
            sc_idx_offset = 0;
            rx_n6MHz = noisy_tx_ofdm_symbol_with_cp_23dot04MHz.*exp(-1i.*2.*pi.*(sc_idx_offset).*t_idx./nfft_23dot04MHz);
            ...
            rx_ofdm_symbol_with_cp_23dot04MHz = rx_n6MHz;
            
            figure;
            plot(0:1:nfft_1dot4MHz-1, abs(fft(rx_n6MHz(len_cp_1dot4MHz+1:end), nfft_1dot4MHz)));
            grid on;            
            
            
            
             %rx_ofdm_symbol_with_cp_23dot04MHz_aux = reshape(rx_ofdm_symbol_with_cp_23dot04MHz, 1, 12*137);
             %spectrumAnalyzer(rx_ofdm_symbol_with_cp_23dot04MHz_aux.');
            
            
%             figure;
%             vphy_test = 1;
%             plot(0:1:nfft_1dot4MHz-1, abs(fft(rx_ofdm_symbol_with_cp_23dot04MHz(len_cp_1dot4MHz+1:end, vphy_test), nfft_1dot4MHz)));
%             grid on;
            ...
            for vphy_idx = 1:1:numOfvPHYs
                ...
                if(ofdm_symbol_idx == 1)
                    ...
                    data_msg = msg((vphy_idx-1)*num_sc_6RB+1:vphy_idx*num_sc_6RB*log2(M));
...
%EbNodB =  SNR - ( 10*log10(modOrd) + 10*log10((numOfvPHYs*num_sc_6RB)/nfft_23dot04MHz) - 10*log10((numOfvPHYs*num_sc_6RB)/nfft_1dot4MHz) );
...
if(0)
    ...
    fileName = sprintf('mcf_tdma_ber_analysis_v5_single_phy_v1_%s.mat', timeStamp);
    save(fileName);
end
