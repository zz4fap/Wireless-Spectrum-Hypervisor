...
%------------------------
% Filter
Hd = filter_mcf_channelizer;

% Instantiate channelizer.
numOfvPHYsIn20MHz = nfft_23dot04MHz/nfft_1dot4MHz;
channelizer = dsp.Channelizer(numOfvPHYsIn20MHz, 'StopbandAttenuation', 1*80, 'NumTapsPerBand', 1*12);
%channelizer = dsp.Channelizer(numOfvPHYsIn20MHz, 'Specification', 'Coefficients');
%channelizer.LowpassCoefficients = Hd.Numerator;
...
%rf_boost = 1.0*ones(1,numOfvPHYs);
rf_boost = [0.1 0.5012 0.7079 0.01 0.3162 0.001 1.0 0.0316 0.1 0.0032 0.7079 0.3162];
...
for vphy_idx = 0:1:numOfvPHYs-1
    ...
    modulated_signal = rf_boost(vphy_idx+1).*modulated_signal;
    ...
    plot(f, 20*log10(abs(tx_23dor04MHz_signal)), 'LineWidth', 1);
...
ylabel('20*log10(|Y|) [dBW]');

if(0)
    legend('vPHY #0   (-20 dB)','vPHY #1   (-6 dB)','vPHY #2   (-3 dB)','vPHY #3   (-40 dB)','vPHY #4   (-10 dB)','vPHY #5   (-60 dB)','vPHY #6   (0 dB)','vPHY #7   (-30 dB)','vPHY #8   (-20 dB)','vPHY #9   (-50 dB)','vPHY #10 (-3 dB)','vPHY #11 (-10 dB)');
    ylim([-65 1])
    yticks([-65 -60 -50 -40 -30 -20 -10 -6 -3 0])
else
...
%     if(plot_enabled)
%             figure(vphy_idx);
%             plot(0:1:nfft_1dot4MHz-1, 20*log10(abs(fft(rx_vphy_ofdm_symbol, nfft_1dot4MHz)).^2));
%             grid on;
