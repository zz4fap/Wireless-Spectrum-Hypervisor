...
numIterSingle = 1e3;
numIter_vector = numIterSingle*ones(1, 16);
...
for snr_idx=1:1:length(SNR)
    ...
    nBits = 0;
    ber_vphy_iter = zeros(numOfvPHYs, numIterSingle);
    parfor iter=1:1:numIter
        
        pilot_signallll = pilot_signal;
        ...
        for ofdm_symbol_idx = 1:1:2
            ...
            for vphy_idx = 0:1:numOfvPHYs-1
                ...
                if(ofdm_symbol_idx==1)
                    s_20MHz_100RB(indexes) = [pilot_signallll(1:num_sc_6RB/2); 0; pilot_signallll((num_sc_6RB/2)+1:end)];
                else
            ...
            ber_vphy_local = zeros(numOfvPHYs, 1);
            for vphy_idx = 1:1:numOfvPHYs
                ...
                if(ofdm_symbol_idx==1)
                    ...
                    if(estimate_channel)
                        channel_estimate(:,vphy_idx) = rx_data_symbols./(pilot_signallll);
                    else
                ...
                if(ofdm_symbol_idx == 1)
                    ...
                    ber_vphy_local(vphy_idx) = ber_vphy_local(vphy_idx) + biterr(demodulated_data, data_msg);
                    
                end
                
            end
            
            ber_vphy_iter(:, iter) = ber_vphy_local;
    ...
    for vphy_idx = 1:1:numOfvPHYs
       ...
       vphy_ber(snr_idx, vphy_idx) = sum(ber_vphy_iter(vphy_idx, :));
...
if(1)
    ...
    fileName = sprintf('mcf_tdma_ber_analysis_v9_%s.mat', timeStamp);
    save(fileName);
end
