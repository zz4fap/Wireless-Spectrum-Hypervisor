...
channelizer = dsp.Channelizer(numOfvPHYsIn20MHz, 'StopbandAttenuation', 180, 'NumTapsPerBand', 120);
[h,w] = freqz(channelizer,'Fs', 23.04e6);
...
for idx=1:1:12
    plot(freq,10*log10(abs(h(:,idx)).^2));
    if(idx==1)
       hold on; 
    end
end
...
if(0)
    ...
    fileName = sprintf('mcf_tdma_mse_analysis_v0_%s.mat', timeStamp);
    save(fileName);
end
