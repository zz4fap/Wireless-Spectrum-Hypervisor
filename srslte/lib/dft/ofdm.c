...
int srslte_ofdm_init_(srslte_ofdm_t *q, srslte_cp_t cp, int symbol_sz, int nof_prb, srslte_dft_dir_t dir, bool set_dc) {
  ...
  // Configure whether DC is set or not.
  srslte_dft_plan_set_dc(&q->fft_plan, set_dc);
...
int srslte_ofdm_rx_init(srslte_ofdm_t *q, srslte_cp_t cp, uint32_t nof_prb) {
  ...
  return srslte_ofdm_init_(q, cp, symbol_sz, nof_prb, SRSLTE_DFT_FORWARD, true);
}
...
int srslte_ofdm_tx_init(srslte_ofdm_t *q, srslte_cp_t cp, uint32_t nof_prb) {
  ...
  // Originally, DC is set to TRUE as the DC subcarrier MUST be equal to 0, however, as it is not a standard implementation
  // where the IFFT of 20 MHz PHY is used to allocate vPHY with smaller BW, then DC MUST not be set to 0 and we MUST make sure
  // each one of the narrower vPHYs have the central subcarrier set to 0.
  ret = srslte_ofdm_init_(q, cp, symbol_sz, nof_prb, SRSLTE_DFT_BACKWARD, false);
...
// ********************** Implementations for McF-TDMA. BEGIN ************************
void srslte_ofdm_tx_longer_sf(srslte_ofdm_t *q, cf_t *input, cf_t *output) {
  uint32_t n, preamble_slot_lenght;
  preamble_slot_lenght = PREAMBLE_SLOT_SIZE_IN_SYMBOLS*(q->symbol_sz + SRSLTE_CP_ISNORM(q->cp)?SRSLTE_CP_LEN_NORM(1, q->symbol_sz):SRSLTE_CP_LEN_EXT(q->symbol_sz));
  for(n = 0; n < 3; n++) {
    if(n == 0) {
      srslte_ofdm_tx_preamble_slot(q, input, output);
    } else {
      srslte_ofdm_tx_slot(q, &input[n*q->nof_re*q->nof_symbols + PREAMBLE_SLOT_SIZE_IN_SYMBOLS*q->nof_re], &output[n*q->slot_sz + preamble_slot_lenght]);
    }
  }
  if(q->freq_shift) {
    srslte_vec_prod_ccc(output, q->shift_buffer, output, (2*q->slot_sz + preamble_slot_lenght));
  }
}

// Performs OFDM modulation on the preamble slot, i.e., the initial 2 OFDM symbols carying PSS and SSS signals for synchronization.
// Transforms input OFDM symbols into output samples.
// Performs FFT on a each symbol and adds CP.
void srslte_ofdm_tx_preamble_slot(srslte_ofdm_t *q, cf_t *input, cf_t *output) {
  uint32_t i, cp_len;
  cp_len = SRSLTE_CP_ISNORM(q->cp)?SRSLTE_CP_LEN_NORM(1, q->symbol_sz):SRSLTE_CP_LEN_EXT(q->symbol_sz);
  for(i = 0; i < PREAMBLE_SLOT_SIZE_IN_SYMBOLS; i++) {
    memcpy(&q->tmp[q->nof_guards], input, q->nof_re * sizeof(cf_t));
    srslte_dft_run_c(&q->fft_plan, q->tmp, &output[cp_len]);
    input += q->nof_re;
    // Add CP.
...
void srslte_ofdm_tx_sf_no_gb(srslte_ofdm_t *q, cf_t *input, cf_t *output) {
  uint32_t n;
  for(n = 0; n < 2; n++) {
    srslte_ofdm_tx_slot_no_gb(q, &input[n*q->fft_plan.size*q->nof_symbols], &output[n*q->slot_sz]);
  }
  if(q->freq_shift) {
    srslte_vec_prod_ccc(output, q->shift_buffer, output, 2*q->slot_sz);
  }
...
/* Transforms input OFDM symbols into output samples.
 * Performs FFT on a each symbol and adds CP.
 * DO NOT consider Guards Bands.
 */
void srslte_ofdm_tx_slot_no_gb(srslte_ofdm_t *q, cf_t *input, cf_t *output) {
  uint32_t i, cp_len;
  for(i = 0; i < q->nof_symbols; i++) {
    cp_len = SRSLTE_CP_ISNORM(q->cp)?SRSLTE_CP_LEN_NORM(i, q->symbol_sz):SRSLTE_CP_LEN_EXT(q->symbol_sz);
    srslte_dft_run_c(&q->fft_plan, input, &output[cp_len]);
    input += q->fft_plan.size;
    // Add CP.
    DEBUG("symbol_sz: %d - cp_len: %d\n",q->symbol_sz,cp_len);
    memcpy(output, &output[q->symbol_sz], cp_len * sizeof(cf_t));
    output += q->symbol_sz + cp_len;
...
void srslte_ofdm_tx_longer_sf_no_gb(srslte_ofdm_t *q, cf_t *input, cf_t *output) {
  uint32_t n, preamble_slot_lenght;
  preamble_slot_lenght = PREAMBLE_SLOT_SIZE_IN_SYMBOLS*(q->symbol_sz + SRSLTE_CP_ISNORM(q->cp)?SRSLTE_CP_LEN_NORM(1, q->symbol_sz):SRSLTE_CP_LEN_EXT(q->symbol_sz));
  for(n = 0; n < 3; n++) {
    if(n == 0) {
      srslte_ofdm_tx_preamble_slot_no_gb(q, input, output);
    } else {
      srslte_ofdm_tx_slot_no_gb(q, &input[n*q->fft_plan.size*q->nof_symbols + PREAMBLE_SLOT_SIZE_IN_SYMBOLS*q->fft_plan.size], &output[n*q->slot_sz + preamble_slot_lenght]);
    }
  }
  if(q->freq_shift) {
    srslte_vec_prod_ccc(output, q->shift_buffer, output, (2*q->slot_sz + preamble_slot_lenght));
...
// Transforms input OFDM symbols into output samples.
// Performs FFT on a each symbol and adds CP.
void srslte_ofdm_tx_preamble_slot_no_gb(srslte_ofdm_t *q, cf_t *input, cf_t *output) {
  uint32_t i, cp_len;
  cp_len = SRSLTE_CP_ISNORM(q->cp)?SRSLTE_CP_LEN_NORM(1, q->symbol_sz):SRSLTE_CP_LEN_EXT(q->symbol_sz);
  for(i = 0; i < PREAMBLE_SLOT_SIZE_IN_SYMBOLS; i++) {
    srslte_dft_run_c(&q->fft_plan, input, &output[cp_len]);
