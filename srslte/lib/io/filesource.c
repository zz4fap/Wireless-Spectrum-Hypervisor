...
int filesource_init(srslte_filesource_t *q, char *filename, srslte_datatype_t type) {
  bzero(q, sizeof(srslte_filesource_t));
  q->f = fopen(filename, "rb");
  if (!q->f) {
    perror("fopen");
    return -1;
  }
...
void filesource_free(srslte_filesource_t *q) {
  if (q->f) {
    fclose(q->f);
  }
  bzero(q, sizeof(srslte_filesource_t));
...
int filesource_read(srslte_filesource_t *q, void *buffer, int nsamples) {

  size_t counter = 0;
  if(q->type == SRSLTE_COMPLEX_SHORT_BIN) {
    counter = fread(buffer, sizeof(_Complex short), nsamples, q->f);
  } else if(q->type == SRSLTE_COMPLEX_FLOAT_BIN) {
    counter = fread(buffer, sizeof(_Complex float), nsamples, q->f);
  } else {
