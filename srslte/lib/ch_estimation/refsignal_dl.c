...
// *****************************************************************************
// Maps a reference signal initialized with srslte_refsignal_cs_init() into an array of subframe symbols.
int srslte_insert_refsignal_into_subframe(srslte_cell_t cell, uint32_t radio_nof_prb, uint32_t channel, uint32_t port_id, cf_t *pilots, cf_t *sf_symbols)
{
  uint32_t i, l;
  uint32_t fidx;
      ...
      sf_symbols                  != NULL)
  {
    for(l = 0; l < srslte_refsignal_cs_nof_symbols(port_id); l++) {
      ...
      fidx = ((srslte_refsignal_cs_v(port_id, l) + (cell.id % 6)) % 6);
      for(i = 0; i < 2*cell.nof_prb; i++) {
        sf_symbols[SRSLTE_RE_IDX2(radio_nof_prb, cell.nof_prb, channel, nsymbol, fidx)] = pilots[SRSLTE_REFSIGNAL_PILOT_IDX(i,l,cell)];
        fidx += SRSLTE_NRE/2;       // 1 reference every 6 RE.
      }
    }
    return SRSLTE_SUCCESS;
...
// Maps the initialized reference signal into into the resource grid.
int srslte_insert_refsignal_into_subframe_with_dc(srslte_cell_t cell, uint32_t radio_fft_len, int dc_pos, uint32_t port_id, cf_t *pilots, cf_t *sf_symbols)
{
  uint32_t i, l;
  uint32_t fidx;
  int init_pos;
  ...
  if( srslte_cell_isvalid(&cell)          &&
      ...
      sf_symbols                  != NULL)
  {
    // Iterate over all the OFDM symbols that have CRS symbols added to it.
    for(l = 0; l < srslte_refsignal_cs_nof_symbols(port_id); l++) {
      uint32_t nsymbol = srslte_refsignal_cs_nsymbol(l, cell.cp, port_id);
      // Compute offset frequency index.
      fidx = ((srslte_refsignal_cs_v(port_id, l) + (cell.id % 6)) % 6);
      // Calculate initial position for CRS symbol.
      init_pos = dc_pos - (((cell.nof_prb*SRSLTE_NRE)/2) + (SRSLTE_NRE/2)) + fidx;
      if(init_pos < 0) {
        init_pos = init_pos + radio_fft_len;
      }
      // Iterate over all 12 CRS symbols added to an OFDM symbol.
      for(i = 0; i < 2*cell.nof_prb; i++) {
        // Add 1 reference symbol to the resource grid every 6 RE at specifc OFDM symbols.
        init_pos = ((init_pos + SRSLTE_NRE/2) % radio_fft_len) + nsymbol*radio_fft_len;
        // Add CRS to resource grid.
        sf_symbols[init_pos] = pilots[SRSLTE_REFSIGNAL_PILOT_IDX(i,l,cell)];
        // Increment one position so that DC is added to the grid.
        if(i == 5) {
          init_pos = init_pos + 1;
        }
      }
