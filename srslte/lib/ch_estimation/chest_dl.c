...
#define CE_CURRENT_SFLEN_RE(nof_prb,cp) SRSLTE_SF_LEN_RE(nof_prb,cp)
...
int srslte_chest_dl_init(srslte_chest_dl_t *q, srslte_cell_t cell)
{
      ...
      srslte_cell_isvalid(&cell))
  {
    ...
    q->tmp_noise = (cf_t*)srslte_vec_malloc(sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));
    if(!q->tmp_noise) {
      perror("malloc");
      goto clean_exit;
    }
    // Initialize memory.
    bzero(q->tmp_noise, sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));

    q->pilot_estimates = (cf_t*)srslte_vec_malloc(sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));
    if (!q->pilot_estimates) {
      perror("malloc");
      goto clean_exit;
    }
    // Initialize memory.
    bzero(q->pilot_estimates, sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));

    q->pilot_estimates_average = (cf_t*)srslte_vec_malloc(sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));
    if (!q->pilot_estimates_average) {
      perror("malloc");
      goto clean_exit;
    }
    // Initialize memory.
    bzero(q->pilot_estimates_average, sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));

    q->pilot_recv_signal = (cf_t*)srslte_vec_malloc(sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));
    if (!q->pilot_recv_signal) {
      perror("malloc");
      goto clean_exit;
    }
    // Initialize memory.
    bzero(q->pilot_recv_signal, sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));

    q->tmp_cfo_estimate = srslte_vec_malloc(sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));
    if (!q->tmp_cfo_estimate) {
      perror("malloc");
      goto clean_exit;
    }
    // Initialize memory.
    bzero(q->tmp_cfo_estimate, sizeof(cf_t) * SRSLTE_REFSIGNAL_MAX_NUM_SF(cell.nof_prb));

    if (srslte_cfo_init_finer(&q->cfocorr, CE_CURRENT_SFLEN_RE(cell.nof_prb, cell.cp))) {
    ...
    q->n  = (float) srslte_symbol_sz(cell.nof_prb);
    q->ns = (float) SRSLTE_CP_NSYMB(cell.cp);
    q->ng = (float) SRSLTE_CP_LEN_NORM(1, q->n);

    q->npilots = SRSLTE_REFSIGNAL_NUM_SF(cell.nof_prb, 0);

    q->cfo_estimate_from_csr_enable = false;
...
void srslte_chest_dl_free(srslte_chest_dl_t *q)
{
  ...
  srslte_cfo_free_finer(&q->cfocorr);
...
int srslte_chest_dl_estimate_port(srslte_chest_dl_t *q, cf_t *input, cf_t *ce, uint32_t sf_idx, uint32_t port_id)
{
  ...
  // If CFO estimation based on CSR signals is enabled.
  if(q->cfo_estimate_from_csr_enable) {
    q->cfo_csr = chest_estimate_cfo(q);
    CHEST_DL_INFO("CSR cfo before: %f [Hz]\n", q->cfo_csr*15000); // Value given in Hz after multiplying by 15000.
  }
...
float chest_estimate_cfo(srslte_chest_dl_t *q) {
  ...
  for(int i = 0; i < 2; i++) {
    srslte_vec_prod_conj_ccc(&q->pilot_estimates[i*q->npilots/4],
                             &q->pilot_estimates[(i+2)*q->npilots/4],
                             &q->tmp_cfo_estimate[i*q->npilots/4],
                             q->npilots/4);
  }
  // Average all angles.
  cf_t sum = srslte_vec_acc_cc(q->tmp_cfo_estimate, q->npilots/2);

  // Compute CFO.
  return -cargf(sum)*q->n/(q->ns*(q->n+q->ng))/2/M_PI;
}
...
void srslte_chest_dl_set_cfo_csr(srslte_chest_dl_t *q, bool flag) {
  q->cfo_estimate_from_csr_enable = flag;
}
