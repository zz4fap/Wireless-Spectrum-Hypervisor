...
int srslte_sch_init(srslte_sch_t *q) {
  return srslte_sch_init_generic(q, 0);
}

int srslte_sch_init_generic(srslte_sch_t *q, uint32_t vphy_id) {
  ...
  if (q) {
    ...
    if (srslte_tcod_init_generic(&q->encoder, SRSLTE_TCOD_MAX_LEN_CB, q->vphy_id)) {
    ...
    srslte_rm_turbo_gentables(q->vphy_id);
...
void srslte_sch_free(srslte_sch_t *q) {
  srslte_rm_turbo_free_tables(q->vphy_id);
                     ...
                     uint8_t *data, uint8_t *e_bits, uint32_t w_offset)
{
      ...
      softbuffer  != NULL)
  {
    ...
    for (i = 0; i < cb_segm->C; i++) {
      ...
      /* Rate matching */
      if (srslte_rm_turbo_tx_lut(q->vphy_id, softbuffer->buffer_b[i], q->cb_in, q->parity_bits,
                     ...
                     void *e_bits, uint8_t *data)
{
  ...
  for (int cb_idx=0;cb_idx<cb_segm->C;cb_idx++)
  {
    ...
    if (softbuffer->cb_crc[cb_idx] == false) {
      ...
      if (q->llr_is_8bit) {
        if (srslte_rm_turbo_rx_lut_8bit(q->vphy_id, &e_bits_b[rp], (int8_t*) softbuffer->buffer_f[cb_idx], n_e2, cb_len_idx, rv)) {
          fprintf(stderr, "Error in rate matching\n");
          return SRSLTE_ERROR;
      }
      } else {
        if (srslte_rm_turbo_rx_lut(q->vphy_id, &e_bits_s[rp], softbuffer->buffer_f[cb_idx], n_e2, cb_len_idx, rv)) {
          fprintf(stderr, "Error in rate matching\n");
        return SRSLTE_ERROR;
        }
      }
      ...
      // Calculate the average number of turbo decoding iterations.
      ...
      cb_segm      != NULL)
  {
    ...
    if (crc_ok) {
      ...
      if (par_rx == par_tx && par_rx) {
        ...
        INFO("vPHY ID: %d - Error in TB parity: par_tx=0x%x, par_rx=0x%x\n", q->vphy_id, par_tx, par_rx);
        q->tb_crc_error++;
        return SRSLTE_ERROR;
      }
    } else {
      INFO("vPHY ID: %d - CB CRC Error.\n", q->vphy_id);
      q->cb_crc_error++;
      return SRSLTE_ERROR;
    }
  } else {
...
int srslte_dlsch_encode2(srslte_sch_t *q, srslte_pdsch_cfg_t *cfg, srslte_softbuffer_tx_t *softbuffer,
  ...
  return encode_tb(q, softbuffer, &cfg->cb_segm, cfg->grant.Qm, cfg->rv,
                   cfg->nbits.nof_bits, data, e_bits);
}
