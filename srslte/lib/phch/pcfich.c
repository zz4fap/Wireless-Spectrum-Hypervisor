...
// *****************************************************************************
// Encodes CFI and maps symbols to the slot.
int srslte_encode_pcfich(srslte_pcfich_t *q, uint32_t cfi, cf_t *slot_symbols[SRSLTE_MAX_PORTS], uint32_t subframe, uint32_t radio_nof_prb, uint32_t channel) {
  int i;

  if(q != NULL && cfi <=  3 && slot_symbols != NULL && subframe < SRSLTE_NSUBFRAMES_X_FRAME) {
    // Set pointers for layermapping & precoding.
    cf_t *x[SRSLTE_MAX_LAYERS];
    cf_t *symbols_precoding[SRSLTE_MAX_PORTS];
    ...
    if(q->cell.nof_ports > 1) {
      srslte_layermap_diversity(q->d, x, q->cell.nof_ports, q->nof_symbols);
      srslte_precoding_diversity(x, symbols_precoding, q->cell.nof_ports, q->nof_symbols / q->cell.nof_ports);
    } else {
      memcpy(q->symbols[0], q->d, q->nof_symbols*sizeof(cf_t));
    }
    ...
    // Mapping to resource elements.
    for(i = 0; i < q->cell.nof_ports; i++) {
      if(srslte_regs_insert_pcfich_into_subframe(q->regs, q->symbols[i], slot_symbols[i], radio_nof_prb, channel) < 0) {
        fprintf(stderr, "Error putting PCHICH resource elements.\n");
        return SRSLTE_ERROR;
      }
    }
...
int srslte_encode_and_map_pcfich_with_dc(srslte_pcfich_t *q, uint32_t cfi, cf_t *slot_symbols[SRSLTE_MAX_PORTS], uint32_t subframe, uint32_t radio_fft_len, int dc_pos) {
  int i;
  ...
  if(q != NULL && cfi <=  3 && slot_symbols != NULL && subframe < SRSLTE_NSUBFRAMES_X_FRAME) {
    ...
    // Mapping to resource elements.
    for(i = 0; i < q->cell.nof_ports; i++) {
      if(srslte_regs_insert_pcfich_into_subframe_with_dc(q->regs, q->symbols[i], slot_symbols[i], radio_fft_len, dc_pos) < 0) {
        fprintf(stderr, "Error putting PCHICH resource elements.\n");
        return SRSLTE_ERROR;
      }
    }
