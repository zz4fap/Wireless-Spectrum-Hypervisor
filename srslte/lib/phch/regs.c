...
#define REG_IDX2(reg, i, nof_prb, channel) ((reg->k[i]+reg->l*nof_prb*SRSLTE_NRE) + (channel*nof_prb*SRSLTE_NRE))
...
// Maps the PCFICH symbols to the resource grid pointed by slot_symbols.
// Returns the number of written symbols, or -1 on error.
int srslte_regs_insert_pcfich_into_subframe(srslte_regs_t *h, cf_t symbols[REGS_PCFICH_NSYM], cf_t *slot_symbols, uint32_t radio_nof_prb, uint32_t channel) {
  uint32_t i;
  srslte_regs_ch_t *rch = &h->pcfich;
  for(i = 0; i < rch->nof_regs && i*REGS_RE_X_REG < REGS_PCFICH_NSYM; i++) {
    regs_insert_reg(rch->regs[i], &symbols[i*REGS_RE_X_REG], slot_symbols, h->cell.nof_prb, radio_nof_prb, channel);
  }
  return i*REGS_RE_X_REG;
}

// Puts one REG data (4 symbols) in the slot symbols array.
int regs_insert_reg(srslte_regs_reg_t *reg, cf_t *reg_data, cf_t *slot_symbols, uint32_t nof_prb, uint32_t radio_nof_prb, uint32_t channel) {
  uint32_t i, idx;
  for(i = 0; i < REGS_RE_X_REG; i++) {
    idx = (reg->k[i]+reg->l*radio_nof_prb*SRSLTE_NRE) + (channel*nof_prb*SRSLTE_NRE);
    slot_symbols[idx] = reg_data[i];
  }
  return REGS_RE_X_REG;
}

// Copy quadruplets to REGs and cyclic shift them, according to the second part of 6.8.5 in 36.211.
int srslte_regs_pdcch_insert_offset(srslte_regs_t *h, cf_t *d, cf_t *slot_symbols, uint32_t start_reg, uint32_t nof_regs, uint32_t radio_nof_prb, uint32_t channel) {
  if(h->cfi_initiated) {
    if(start_reg + nof_regs <= h->pdcch[h->cfi].nof_regs) {
      uint32_t i, k;
      k = 0;
      for(i = start_reg; i < start_reg+nof_regs; i++) {
        regs_insert_reg(h->pdcch[h->cfi].regs[i], &d[k], slot_symbols, h->cell.nof_prb, radio_nof_prb, channel);
        k += 4;
      }
      return k;
    } else {
...
// Maps the PCFICH symbols to the resource grid pointed by slot_symbols and respect DC position.
// Returns the number of written symbols, or -1 on error.
int srslte_regs_insert_pcfich_into_subframe_with_dc(srslte_regs_t *h, cf_t symbols[REGS_PCFICH_NSYM], cf_t *slot_symbols, uint32_t radio_fft_len, int dc_pos) {
  uint32_t i;
  srslte_regs_ch_t *rch = &h->pcfich;
  // Calculate position of first useful subcarrier of a vPHY.
  int init_pos = dc_pos - ((h->cell.nof_prb*SRSLTE_NRE)/2);
  if(init_pos < 0) {
    init_pos = init_pos + radio_fft_len;
  }
  // Iterate over groups of PCFICH REGs.
  for(i = 0; i < rch->nof_regs && i*REGS_RE_X_REG < REGS_PCFICH_NSYM; i++) {
    regs_insert_reg_with_dc(rch->regs[i], &symbols[i*REGS_RE_X_REG], slot_symbols, init_pos, radio_fft_len, h->cell.nof_prb);
  }
  return i*REGS_RE_X_REG;
}

// Copy quadruplets to REGs and cyclic shift them, according to the second part of 6.8.5 in 36.211.
int srslte_regs_pdcch_insert_offset_with_dc(srslte_regs_t *h, cf_t *d, cf_t *slot_symbols, uint32_t start_reg, uint32_t nof_regs, uint32_t radio_fft_len, int dc_pos) {
  if(h->cfi_initiated) {
    if(start_reg + nof_regs <= h->pdcch[h->cfi].nof_regs) {
      uint32_t i, k;
      k = 0;
      ...
      // Iterate over groups of PDCCH REGs.
      for(i = start_reg; i < start_reg+nof_regs; i++) {
        regs_insert_reg_with_dc(h->pdcch[h->cfi].regs[i], &d[k], slot_symbols, init_pos, radio_fft_len, h->cell.nof_prb);
        k += 4;
      }
      return k;
    } else {
...
// Puts one REG data (4 symbols) in the slot symbols array and respect DC subcarrier position.
int regs_insert_reg_with_dc(srslte_regs_reg_t *reg, cf_t *reg_data, cf_t *slot_symbols, uint32_t init_pos, uint32_t radio_fft_len, uint32_t nof_prb) {
  uint32_t i, idx;
  for(i = 0; i < REGS_RE_X_REG; i++) {
    // Calculate PCFICH symbol position.
    idx = (init_pos + reg->k[i]) % radio_fft_len;
    if(reg->k[i] >= ((nof_prb*SRSLTE_NRE)/2)) {
      idx = idx + 1;
    }
    idx = idx + reg->l*radio_fft_len;
    // Insert symbol to resource grid.
    slot_symbols[idx] = reg_data[i];
  }
  return REGS_RE_X_REG;
}
