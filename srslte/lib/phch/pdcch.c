 ...
 * @param[in] cfi Control Format Indicator
 */
int srslte_encode_pdcch(srslte_pdcch_t *q, srslte_dci_msg_t *msg, srslte_dci_location_t location, uint16_t rnti, cf_t *sf_symbols[SRSLTE_MAX_PORTS], uint32_t nsubframe, uint32_t cfi, uint32_t radio_nof_prb, uint32_t channel) {
  ...
  if(q != NULL && sf_symbols != NULL && nsubframe <  10 && cfi >  0 && cfi < 4 && srslte_dci_location_isvalid(&location)) {
    ...
    if(location.ncce + PDCCH_FORMAT_NOF_CCE(location.L) <= q->nof_cce && msg->nof_bits < SRSLTE_DCI_MAX_BITS - 16) {
      DEBUG("Encoding DCI: Nbits: %d, E: %d, nCCE: %d, L: %d, RNTI: 0x%x\n", msg->nof_bits, e_bits, location.ncce, location.L, rnti);
      ...
      // Mapping to resource elements.
      for(i = 0; i < q->cell.nof_ports; i++) {
        srslte_regs_pdcch_insert_offset(q->regs, q->symbols[i], sf_symbols[i], location.ncce * 9, PDCCH_FORMAT_NOF_REGS(location.L), radio_nof_prb, channel);
      }
 ...
 * @param[in] cfi Control Format Indicator
 */
int srslte_encode_and_map_pdcch_with_dc(srslte_pdcch_t *q, srslte_dci_msg_t *msg, srslte_dci_location_t location, uint16_t rnti, cf_t *sf_symbols[SRSLTE_MAX_PORTS], uint32_t nsubframe, uint32_t cfi, uint32_t radio_fft_len, int dc_pos) {
  ...
  if(q != NULL && sf_symbols != NULL && nsubframe <  10 && cfi >  0 && cfi < 4 && srslte_dci_location_isvalid(&location)) {
    ...
    if(location.ncce + PDCCH_FORMAT_NOF_CCE(location.L) <= q->nof_cce && msg->nof_bits < SRSLTE_DCI_MAX_BITS - 16) {
      ...
      // Mapping to resource elements.
      for(i = 0; i < q->cell.nof_ports; i++) {
        srslte_regs_pdcch_insert_offset_with_dc(q->regs, q->symbols[i], sf_symbols[i], location.ncce * 9, PDCCH_FORMAT_NOF_REGS(location.L), radio_fft_len, dc_pos);
      }
