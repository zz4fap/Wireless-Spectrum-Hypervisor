...
int srslte_pdsch_init(srslte_pdsch_t *q, srslte_cell_t cell) {
  return srslte_pdsch_init_generic(q, cell, 0);
}

/** Initializes the PDSCH transmitter and receiver */
int srslte_pdsch_init_generic(srslte_pdsch_t *q, srslte_cell_t cell, uint32_t vphy_id) {
 ...
 if (q                         != NULL                  &&
     srslte_cell_isvalid(&cell))
  {
    ...
    srslte_sch_init_generic(&q->dl_sch, q->vphy_id);
...
// *****************************************************************************
int srslte_encode_pdsch(srslte_pdsch_t *q,
                        srslte_pdsch_cfg_t *cfg, srslte_softbuffer_tx_t *softbuffer,
                        uint8_t *data, cf_t *sf_symbols[SRSLTE_MAX_PORTS], uint32_t radio_nof_prb, uint32_t channel) {
  if(q != NULL && data != NULL && cfg != NULL) {
    if(q->rnti_is_set) {
      return srslte_pdsch_rnti_encode(q, cfg, softbuffer, data, q->rnti, sf_symbols, radio_nof_prb, channel);
    } else {
      fprintf(stderr, "Must call srslte_pdsch_set_rnti() to set the encoder/decoder RNTI\n");
      return SRSLTE_ERROR;
    }
...
// Converts the PDSCH data bits to symbols mapped to the slot ready for transmission.
int srslte_pdsch_rnti_encode(srslte_pdsch_t *q,
                             srslte_pdsch_cfg_t *cfg, srslte_softbuffer_tx_t *softbuffer,
  ...
  if(rnti != q->rnti) {
    ...
    int r = srslte_pdsch_seq_encode(q, cfg, softbuffer, data, &seq, sf_symbols, radio_nof_prb, channel);
    srslte_sequence_free(&seq);
    return r;
  } else {
    return srslte_pdsch_seq_encode(q, cfg, softbuffer, data, &q->seq[cfg->sf_idx], sf_symbols, radio_nof_prb, channel);
  }
}

int srslte_pdsch_seq_encode(srslte_pdsch_t *q,
                            srslte_pdsch_cfg_t *cfg, srslte_softbuffer_tx_t *softbuffer,
  ...
  if(q != NULL && cfg != NULL) {
    ...
    // mapping to resource elements.
    for(i = 0; i < q->cell.nof_ports; i++) {
      srslte_pdsch_insert_into_subframe(q, q->symbols[i], sf_symbols[i], &cfg->grant, cfg->nbits.lstart, cfg->sf_idx, radio_nof_prb, channel);
    }
...
/**
 * Puts PDSCH into subframe.
 *
 * Returns the number of symbols written to sf_symbols.
 *
 * See TS 36.211 10.3 section 6.3.5
 */
int srslte_pdsch_insert_into_subframe(srslte_pdsch_t *q, cf_t *symbols, cf_t *sf_symbols, srslte_ra_dl_grant_t *grant, uint32_t lstart, uint32_t subframe, uint32_t radio_nof_prb, uint32_t channel) {
  return srslte_pdsch_cp_insert_into_subframe(q, symbols, sf_symbols, grant, lstart, subframe, true, radio_nof_prb, channel);
}

int srslte_pdsch_cp_insert_into_subframe(srslte_pdsch_t *q, cf_t *input, cf_t *output, srslte_ra_dl_grant_t *grant, uint32_t lstart_grant, uint32_t nsubframe, bool put, uint32_t radio_nof_prb, uint32_t channel) {
  uint32_t s, n, l, lp, lstart, lend, nof_refs;
  bool is_pbch, is_sss;
  cf_t *in_ptr = input, *out_ptr = output;
  uint32_t offset = 0;
  ...
  for(s = 0; s < 2; s++) { // slot counter.
    for(l = 0; l < SRSLTE_CP_NSYMB(q->cell.cp); l++) { // symbol in slot counter.
      for(n = 0; n < q->cell.nof_prb; n++) { // resource block counter.
        ...
        if(grant->prb_idx[s][n]) {
          ...
          lp = l + s * SRSLTE_CP_NSYMB(q->cell.cp);
          if(put) {
            out_ptr = &output[(lp * radio_nof_prb + n + channel*q->cell.nof_prb) * SRSLTE_NRE];
          } else {
            in_ptr =  &input[(lp * radio_nof_prb + n + channel*q->cell.nof_prb) * SRSLTE_NRE];
          }
          // This is a symbol in a normal PRB with or without references.
          if(l >= lstart && l < lend) {
            if(SRSLTE_SYMBOL_HAS_REF(l, q->cell.cp, q->cell.nof_ports)) {
...
int srslte_encode_and_map_pdsch_with_dc(srslte_pdsch_t *q,
                        srslte_pdsch_cfg_t *cfg, srslte_softbuffer_tx_t *softbuffer,
                        uint8_t *data, cf_t *sf_symbols[SRSLTE_MAX_PORTS], uint32_t radio_fft_len, int dc_pos) {
  if(q != NULL && data != NULL && cfg != NULL) {
    if(q->rnti_is_set) {
      return srslte_pdsch_rnti_encode_and_map_with_dc(q, cfg, softbuffer, data, q->rnti, sf_symbols, radio_fft_len, dc_pos);
    } else {
      fprintf(stderr, "Must call srslte_pdsch_set_rnti() to set the encoder/decoder RNTI\n");
      return SRSLTE_ERROR;
    }
...
// Converts the PDSCH data bits to symbols mapped to the slot ready for transmission.
int srslte_pdsch_rnti_encode_and_map_with_dc(srslte_pdsch_t *q,
                             srslte_pdsch_cfg_t *cfg, srslte_softbuffer_tx_t *softbuffer,
                             uint8_t *data, uint16_t rnti, cf_t *sf_symbols[SRSLTE_MAX_PORTS], uint32_t radio_fft_len, int dc_pos) {
  if(rnti != q->rnti) {
    srslte_sequence_t seq;
    if(srslte_sequence_pdsch(&seq, rnti, 0, 2 * cfg->sf_idx, q->cell.id, cfg->nbits.nof_bits)) {
      return SRSLTE_ERROR;
    }
    int r = srslte_pdsch_seq_encode_and_map_with_dc(q, cfg, softbuffer, data, &seq, sf_symbols, radio_fft_len, dc_pos);
    srslte_sequence_free(&seq);
    return r;
  } else {
    return srslte_pdsch_seq_encode_and_map_with_dc(q, cfg, softbuffer, data, &q->seq[cfg->sf_idx], sf_symbols, radio_fft_len, dc_pos);
  }
}

int srslte_pdsch_seq_encode_and_map_with_dc(srslte_pdsch_t *q,
                            srslte_pdsch_cfg_t *cfg, srslte_softbuffer_tx_t *softbuffer,
                            uint8_t *data, srslte_sequence_t *seq, cf_t *sf_symbols[SRSLTE_MAX_PORTS], uint32_t radio_fft_len, int dc_pos) {
  int i;
  // Set pointers for layermapping & precoding.
  cf_t *x[SRSLTE_MAX_LAYERS];
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  ...
  if(q != NULL && cfg != NULL) {
    ...
    // mapping to resource elements.
    for(i = 0; i < q->cell.nof_ports; i++) {
      srslte_pdsch_map_into_resource_grid_with_dc(q, q->symbols[i], sf_symbols[i], &cfg->grant, cfg->nbits.lstart, cfg->sf_idx, radio_fft_len, dc_pos);
    }
 ...
 * Returns the number of symbols written to sf_symbols.
 *
 * Loosely based on TS 36.211 10.3 section 6.3.5
 */
int srslte_pdsch_map_into_resource_grid_with_dc(srslte_pdsch_t *q, cf_t *symbols, cf_t *sf_symbols, srslte_ra_dl_grant_t *grant, uint32_t lstart, uint32_t subframe, uint32_t radio_fft_len, int dc_pos) {
  return srslte_pdsch_cp_map_into_resource_grid_with_dc(q, symbols, sf_symbols, grant, lstart, subframe, true, radio_fft_len, dc_pos);
}

int srslte_pdsch_cp_map_into_resource_grid_with_dc(srslte_pdsch_t *q, cf_t *input, cf_t *output, srslte_ra_dl_grant_t *grant, uint32_t lstart_grant, uint32_t nsubframe, bool put, uint32_t radio_fft_len, int dc_pos) {
  uint32_t s, n, l, lp, lstart, lend, nof_refs;
  bool is_pbch, is_sss;
  cf_t *in_ptr = input, *out_ptr = output;
  uint32_t offset = 0;
  int init_pos = 0, pos = 0, pos_ = 0;
  ...
  init_pos = (dc_pos - ((q->cell.nof_prb*SRSLTE_NRE)/2));
  if(init_pos < 0) {
  ...
  for(s = 0; s < 2; s++) { // slot counter.
    for(l = 0; l < SRSLTE_CP_NSYMB(q->cell.cp); l++) { // symbol in slot counter.
      ...
      // Get OFDM symbol number within the subframe, i.e., for normal CP a value between 0 and 13.
      lp = l + s * SRSLTE_CP_NSYMB(q->cell.cp);
      ...
      for(n = 0; n < q->cell.nof_prb; n++) { // resource block counter.
        ...
        if(grant->prb_idx[s][n]) {
          ...
          if(put) {
            pos_ = ((init_pos + n*SRSLTE_NRE) % radio_fft_len);
            if(n >= 3) {
              pos_++;
            }
            pos = pos_ + lp*radio_fft_len;
            out_ptr = &output[pos];
          } else {
            pos_ = ((init_pos + n*SRSLTE_NRE) % radio_fft_len);
            if(n >= 3) {
          ...
          if(l >= lstart && l < lend) {
            if(SRSLTE_SYMBOL_HAS_REF(l, q->cell.cp, q->cell.nof_ports)) {
              ...
              prb_cp_ref_with_dc(&in_ptr, &out_ptr, offset, nof_refs, nof_refs, put, 1, radio_fft_len, pos_);
            } else {
              prb_cp_with_dc(&in_ptr, &out_ptr, 1, radio_fft_len, pos_);
            }
          }
          // This is a symbol in a PRB with PBCH or Synchronization signals (PSS/SSS).
          // If the number or total PRB is odd, half of the the PBCH or PSS/SSS will fall into the symbol.
          if((q->cell.nof_prb % 2) && ((is_pbch && l < lstart) || (is_sss && l >= lend))) {
            if(n == q->cell.nof_prb / 2 - 3) {
              if(SRSLTE_SYMBOL_HAS_REF(l, q->cell.cp, q->cell.nof_ports)) {
                prb_cp_ref_with_dc(&in_ptr, &out_ptr, offset, nof_refs, nof_refs/2, put, 1, radio_fft_len, pos_);
              } else {
                prb_cp_half_with_dc(&in_ptr, &out_ptr, 1, radio_fft_len, pos_);
              }
            } else if(n == q->cell.nof_prb / 2 + 3) {
              if(put) {
                out_ptr += 6;
