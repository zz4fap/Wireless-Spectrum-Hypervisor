...
//********************************** DcF-TDMA **********************************
//******************************************************************************
int srslte_cexptab_init_finer(srslte_cexptab_t *h, uint32_t size) {
  uint32_t i;

  h->size = size;
  h->tab = srslte_vec_malloc(sizeof(cf_t)*size);
  if(h->tab) {
    for(i = 0; i < size; i++) {
      h->tab[i] = cexpf((_Complex_I * 2 * M_PI * (float) i) / ((float)size));
    }
    return SRSLTE_SUCCESS;
  } else {
    return SRSLTE_ERROR;
...
void srslte_cexptab_free_finer(srslte_cexptab_t *h) {
  if(h->tab) {
    free(h->tab);
  }
  bzero(h, sizeof(srslte_cexptab_t));
...
void srslte_cexptab_gen_finer(srslte_cexptab_t *h, cf_t *x, float freq, uint32_t len) {
  uint32_t i;
  uint32_t idx;
  float phase_inc = freq * h->size;
  float phase = 0;

  //printf("######### phase_inc: %1.4f, freq: %1.4f - h->size: %d - len: %d\n", phase_inc, freq, h->size, len);
