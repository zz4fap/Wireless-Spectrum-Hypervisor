...
void srslte_vec_sc_prod_cfc_with_dc(const cf_t *x, const float h, cf_t *z, uint32_t nof_symb, uint32_t radio_fft_len, uint32_t vphy_fft_len, uint32_t vphy_nof_prb, uint32_t channel) {
  uint32_t idx = 0;
  int nof_mul = 0, dc_pos = 0, init_pos = 0;
  ...
  for(uint32_t symb = 0; symb < 2*nof_symb; symb++) { // loop over OFDM symbols.
    if((init_pos + (vphy_nof_prb*SRSLTE_NRE) + 1) <= radio_fft_len) {
      // Index pointing to the exact location of the start of this vPHY.
      idx = init_pos + symb*radio_fft_len;
      // Multiply all subcarrier including the DC subcarrier.
      srslte_vec_sc_prod_cfc_simd(&x[idx], h, &z[idx], ((vphy_nof_prb*SRSLTE_NRE)+1));
    } else {
      // Index pointing to the exact location of the start of this vPHY.
      idx = init_pos + symb*radio_fft_len;
      // Calculate the number of element to be multiplied.
      nof_mul = radio_fft_len - init_pos;
      // Multiply everything until the end of the current OFDM symbol.
      srslte_vec_sc_prod_cfc_simd(&x[idx], h, &z[idx], nof_mul);
      // Calculate the number of remaining subcarriers to be multiplied.
      nof_mul = ((vphy_nof_prb*SRSLTE_NRE) + 1) - nof_mul;
      // Multiply the remaining subcarriers starting from index 0.
      srslte_vec_sc_prod_cfc_simd(&x[0 + symb*radio_fft_len], h, &z[0 + symb*radio_fft_len], nof_mul);
    }
  }
}

void srslte_vec_prod_cfc_with_dc(const cf_t *x, const float *y, cf_t *z, uint32_t nof_symb, uint32_t radio_fft_len, uint32_t vphy_fft_len, uint32_t vphy_nof_prb, uint32_t channel) {
  uint32_t idx;
  ...
  int init_pos = dc_pos - ((vphy_nof_prb*SRSLTE_NRE)/2);
  if(init_pos < 0) {
    init_pos += radio_fft_len;
  }
  ...
  for(uint32_t symb = 0; symb < 2*nof_symb; symb++) { // loop over OFDM symbols.
    if((init_pos + (vphy_nof_prb*SRSLTE_NRE) + 1) <= radio_fft_len) {
      // Index pointing to the exact location of the start of this vPHY.
      idx = init_pos + symb*radio_fft_len;
      // Multiply half of the subcarriers. (lower part)
      srslte_vec_prod_cfc_simd(&x[idx], &y[0], &z[idx], ((vphy_nof_prb*SRSLTE_NRE)/2));
      // Multiply half of the subcarriers. (upper part)
      idx += (vphy_nof_prb*SRSLTE_NRE)/2 + 1;
      srslte_vec_prod_cfc_simd(&x[idx], &y[(vphy_nof_prb*SRSLTE_NRE)/2], &z[idx], ((vphy_nof_prb*SRSLTE_NRE)/2));
    } else {
      // Index pointing to the exact location of the start of this vPHY.
      idx = init_pos + symb*radio_fft_len;
      // Calculate the number of element to be multiplied.
      nof_mul = radio_fft_len - init_pos;
      // Multiply everything until the end of the current OFDM symbol.
      srslte_vec_prod_cfc_simd(&x[idx], &y[0], &z[idx], nof_mul);
      // Calculate the number of remaining subcarriers to be multiplied.
      nof_mul = (vphy_nof_prb*SRSLTE_NRE) - nof_mul;
      // Multiply the remaining subcarriers starting from index 1.
      srslte_vec_prod_cfc_simd(&x[1 + symb*radio_fft_len], &y[(vphy_nof_prb*SRSLTE_NRE)/2], &z[1 + symb*radio_fft_len], nof_mul);
    }
  }
}

// Applies windowing and move head and tail GB subcarriers to their respective positions.
void srslte_vec_prod_cfc_with_dc_liq_window(const cf_t *x, const float *y, cf_t *z, uint32_t nof_symb, uint32_t radio_fft_len, uint32_t vphy_fft_len, uint32_t vphy_nof_prb, uint32_t channel, uint32_t gb_len) {
  uint32_t idx;
  ...
  for(uint32_t symb = 0; symb < 2*nof_symb; symb++) { // loop over OFDM symbols.
    ...
    // Copy head and tail guard-bands.
    for(uint32_t re = 0; re < gb_len; re++) {
      idx = init_pos - gb_len + re + symb*radio_fft_len;
      z[idx] = (0.4082 + 0.4082*I)*y[re];
      idx = ((init_pos + vphy_nof_prb*SRSLTE_NRE + 1) % radio_fft_len) + symb*radio_fft_len;
      z[idx] = (0.4082 + 0.4082*I)*y[re + vphy_nof_prb*SRSLTE_NRE + gb_len];
    }
  }
}

void srslte_vec_sc_prod_cfc2(cf_t *x, float h, cf_t *z, uint32_t nof_symb, uint32_t nof_prb, uint32_t radio_nof_prb, uint32_t channel) {
  uint32_t idx;
  for(uint32_t symb = 0; symb < 2*nof_symb; symb++) {
    idx = symb*radio_nof_prb*SRSLTE_NRE + channel*nof_prb*SRSLTE_NRE;
    srslte_vec_sc_prod_cfc_simd(&x[idx], h, &z[idx], nof_prb*SRSLTE_NRE);
  }
}

void srslte_vec_prod_cfc2(cf_t *x, float *y, cf_t *z, uint32_t nof_symb, uint32_t nof_prb, uint32_t radio_nof_prb, uint32_t channel) {
  uint32_t idx;
  for(uint32_t symb = 0; symb < 2*nof_symb; symb++) { // loop over subframe ofdm symbols.
    idx = symb*radio_nof_prb*SRSLTE_NRE + channel*nof_prb*SRSLTE_NRE;
    srslte_vec_prod_cfc_simd(&x[idx], y, &z[idx], nof_prb*SRSLTE_NRE);
  }
}

// Applies windowing and move head and tail GB subcarriers to their respective positions.
void srslte_vec_prod_cfc3(cf_t *x, float *y, cf_t *z, uint32_t nof_symb, uint32_t nof_prb, uint32_t radio_nof_prb, uint32_t channel, uint32_t gb_len) {
  uint32_t idx;
  for(uint32_t symb = 0; symb < 2*nof_symb; symb++) { // loop over subframe ofdm symbols.
    idx = symb*radio_nof_prb*SRSLTE_NRE + channel*nof_prb*SRSLTE_NRE;
    srslte_vec_prod_cfc_simd(&x[idx], &y[gb_len], &z[idx], nof_prb*SRSLTE_NRE);
    // Copy head and tail guard-bands.
    for(uint32_t re = 0; re < gb_len; re++) {
      idx = re + symb*radio_nof_prb*SRSLTE_NRE + channel*nof_prb*SRSLTE_NRE;
      z[idx - gb_len] = (0.4082 + 0.4082*I)*y[re];
      z[idx + nof_prb*SRSLTE_NRE] = (0.4082 + 0.4082*I)*y[re + nof_prb*SRSLTE_NRE + gb_len];
    }
  }
}
