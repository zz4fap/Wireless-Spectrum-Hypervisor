...
int srslte_cp_synch_init(srslte_cp_synch_t *q, uint32_t symbol_sz)
{
  ...
  q->cp_len0 = SRSLTE_CP_LEN_NORM(0,symbol_sz);
  q->cp_len  = SRSLTE_CP_LEN_NORM(1,symbol_sz);

  q->corr = (cf_t*)srslte_vec_malloc(sizeof(cf_t) * q->symbol_sz);
  if (!q->corr) {
    perror("malloc");
    return SRSLTE_ERROR;
  }
...
cf_t srslte_cp_corr(srslte_cp_synch_t *q, cf_t *input, uint32_t nof_symbols) {
  cf_t corr = 0;
  cf_t *inputPtr = input;
  for(int n = 0; n < nof_symbols; n++) {
    uint32_t cplen = (n%7) ? q->cp_len:q->cp_len0;
    corr += srslte_vec_dot_prod_conj_ccc(inputPtr, &inputPtr[q->symbol_sz], cplen);
    inputPtr += (q->symbol_sz+cplen);
  }
  corr = corr/nof_symbols;
  return corr;
}
