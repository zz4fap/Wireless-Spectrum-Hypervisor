...
static void extract_sch_pair_combining(srslte_sss_synch_t *q, uint32_t nof_prb, cf_t *input0, cf_t *input1, cf_t *ce, cf_t y[2][SRSLTE_SCH_N]) {
  ...
  // Channel Equalization (PSS-based channel estimates) can be applied to the middle SCH sequence as it is beside PSS.
  if(ce) {
  ...
  if(nof_prb != 6) {
    for(int i = 0; i < SRSLTE_SCH_N; i++) {
      y[0][i] = (input_fft0[q->fft_size/2-(SRSLTE_SCH_N + (SRSLTE_SCH_N/2 + 1)) + 3 * i]     + input_fft1[q->fft_size/2-SRSLTE_SCH_N + 2 * i])/2.0;
      y[1][i] = (input_fft0[q->fft_size/2-(SRSLTE_SCH_N + (SRSLTE_SCH_N/2 + 1)) + 3 * i + 1] + input_fft1[q->fft_size/2-SRSLTE_SCH_N + 2 * i + 1])/2.0;
    }
  } else {
    for(int i = 0; i < SRSLTE_SCH_N; i++) {
...
#if(0)
  for(uint32_t k = 0; k < SRSLTE_SCH_N; k++) {
    printf("%d \t y00: %+1.4f,%+1.4f \t y01: %+1.4f,%+1.4f \t y0comb: %+1.4f,%+1.4f\n", k, creal(input_fft0[q->fft_size/2-(SRSLTE_SCH_N + (SRSLTE_SCH_N/2 + 1)) + 3 * k]), cimag(input_fft0[q->fft_size/2-(SRSLTE_SCH_N + (SRSLTE_SCH_N/2 + 1)) + 3 * k]), creal(input_fft1[q->fft_size/2-SRSLTE_SCH_N + 2 * k]), cimag(input_fft1[q->fft_size/2-SRSLTE_SCH_N + 2 * k]), creal(y[0][k]), cimag(y[0][k]));
  }
  printf("\n\n");
  for(uint32_t k = 0; k < SRSLTE_SCH_N; k++) {
    printf("%d \t y10: %+1.4f,%+1.4f \t y11: %+1.4f,%+1.4f \t y1comb: %+1.4f,%+1.4f\n", k, creal(input_fft0[q->fft_size/2-(SRSLTE_SCH_N + (SRSLTE_SCH_N/2 + 1)) + 3 * k + 1]), cimag(input_fft0[q->fft_size/2-(SRSLTE_SCH_N + (SRSLTE_SCH_N/2 + 1)) + 3 * k + 1]), creal(input_fft1[q->fft_size/2-SRSLTE_SCH_N + 2 * k + 1]), cimag(input_fft1[q->fft_size/2-SRSLTE_SCH_N + 2 * k + 1]), creal(y[1][k]), cimag(y[1][k]));
  }
#endif
...
// Find m0 and m1 values for SCH signal sequence added to the front of the subframe (symbols 0 or 1).
// Use Receive combing to improve the reception ratio.
// We send the same sequence twice but in different parts of the first subframe.
