...
//********************************** DcF-TDMA **********************************
//******************************************************************************
#define BASIS_SIGNAL_CYCLE_LEN 76800

int srslte_cfo_init_finer(srslte_cfo_t *h, uint32_t nsamples) {
  int ret = SRSLTE_ERROR;
  bzero(h, sizeof(srslte_cfo_t));

  if(srslte_cexptab_init_finer(&h->tab, BASIS_SIGNAL_CYCLE_LEN)) {
    goto clean;
  }
  h->cur_cexp = srslte_vec_malloc(sizeof(cf_t)*nsamples);
  if(!h->cur_cexp) {
  ...
  h->last_freq = 0;
  h->nsamples = nsamples;
  h->fft_size = 384.0; // value just to initialize, i.e., default value.
  srslte_cexptab_gen_finer(&h->tab, h->cur_cexp, h->last_freq, h->nsamples);

  ret = SRSLTE_SUCCESS;
clean:
  if(ret == SRSLTE_ERROR) {
    srslte_cfo_free_finer(h);
  }
  return ret;
}

void srslte_cfo_free_finer(srslte_cfo_t *h) {
  srslte_cexptab_free_finer(&h->tab);
  if(h->cur_cexp) {
    free(h->cur_cexp);
  }
  bzero(h, sizeof(srslte_cfo_t));
...
void srslte_cfo_set_fft_size_finer(srslte_cfo_t *h, uint32_t size) {
  h->fft_size = (float)size;
}

void srslte_cfo_correct_finer(srslte_cfo_t *h, cf_t *input, cf_t *output, float freq) {
  if(fabs(h->last_freq - freq) > h->tol) {
    h->last_freq = freq;
    srslte_cexptab_gen_finer(&h->tab, h->cur_cexp, h->last_freq, h->nsamples);
    DEBUG("CFO generating new table for frequency %1.4f\n", freq*h->fft_size*15000);
  }
  srslte_vec_prod_ccc(h->cur_cexp, input, output, h->nsamples);
}

void srslte_cfo_set_tol_finer(srslte_cfo_t *h, float tol) {
  h->tol = tol;
}
