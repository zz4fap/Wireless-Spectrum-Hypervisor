                                 ...
                                 int (recv_callback)(void*, void*, uint32_t,srslte_timestamp_t*, size_t), int initial_subframe_index)
{
      ...
      srslte_nofprb_isvalid(nof_prb))
  {
    ...
    for(uint32_t i = 0; i < NUMBER_OF_SUBFRAME_BUFFERS; i++) {
                        ...
                        bool enable_cfo_correction)
{
      ...
      recv_callback                     != NULL)
  {
    ...
    UE_SYNC_DEBUG("CFO Correction: %s (%d)\n", enable_cfo_correction==true ? "Enabled":"Disabled", enable_cfo_correction);
...
void srslte_ue_sync_free_except_reentry(srslte_ue_sync_t *q) {
  UE_SYNC_INFO("vPHY Rx ID: %d - Freeing input buffer\n", q->vphy_id);
...
static int find_peak_ok(srslte_ue_sync_t *q, size_t channel) {
  ...
  if(q->frame_find_cnt >= q->nof_avg_find_frames || (q->peak_idx-q->frame_len) < 2*q->fft_size) {
    ...
    // If all the IQ samples of a subframe are already contained in the current buffer, then there is no need to align it to the begining of thebuffer, we just use the position of the subframe start.
    ...
    if(q->file_mode) {
      ...
      switch(q->state) {
        case SF_FIND:
        {
          ...
          //if(sync_ret == SRSLTE_SYNC_FOUND)
          //  PHY_PROFILLING_AVG3("Avg. find peak time: %f - min: %f - max: %f - max counter %d - diff >= 1ms: %d - total counter: %d - perc: %f\n",helpers_profiling_diff_time(start_time), 1.0, 10000);
          switch(sync_ret) {
            case SRSLTE_SYNC_ERROR:
              ret = SRSLTE_ERROR;
          ...
          if(q->correct_cfo && sync_ret == SRSLTE_SYNC_FOUND && ret == 1) {
            ...
            if(fabs(cfo_estimate) > 0.01) { // Only correct if CFO is greater than 150 Hz (i.e., 0.01).
...
int srslte_ue_sync_allocate_subframe_buffer(srslte_ue_sync_t *q) {
   for(uint32_t i = 0; i < NUMBER_OF_SUBFRAME_BUFFERS; i++) {
      ...
      bzero(q->input_buffer[i], NUMBER_OF_SUBFRAMES_TO_STORE*q->frame_len*sizeof(cf_t));
   }
   return SRSLTE_SUCCESS;
}
...
// ******************************** (BEGIN) ************************************
//------------------------------------------------------------------------------
// ####################### Frame structure II (BEGIN) ###########################
static inline int receive_samples_from_hyper_rx_frame_type_two(srslte_ue_sync_t *q) {
  ...
  // Get N subframes from the USRP getting more samples and keeping the previous samples, if any.
  if(q->recv_callback_hyper_rx(q->stream, &q->input_buffer[q->subframe_buffer_counter][num_of_samples_to_stay], offset) < 0) {
    UE_SYNC_ERROR("Error receive_samples at receive_samples() - subframe_buffer_counter: %d - num_of_samples_to_stay: %d - offset: %d\n", q->subframe_buffer_counter, num_of_samples_to_stay, offset);
    return SRSLTE_ERROR;
  }
...
static int find_peak_ok_hyper_rx_frame_type_two(srslte_ue_sync_t *q) {
  ...
  if(q->frame_find_cnt >= q->nof_avg_find_frames || (q->peak_idx-q->frame_len) < 2*q->fft_size) {
    ...
    if(num_of_missing_samples == 0) {
      ...
      memcpy((uint8_t*)q->input_buffer[q->subframe_buffer_counter], (uint8_t*)(q->input_buffer[q->subframe_buffer_counter]+offset), num_of_samples_to_copy*sizeof(cf_t));
      // We read samples if there still are samples to be read from USRP in order to complete the subframe IQ samples.
      if(q->recv_callback_hyper_rx(q->stream, (q->input_buffer[q->subframe_buffer_counter]+num_of_samples_to_copy), num_of_missing_samples) < 0) {
        return SRSLTE_ERROR;
      }
    }
...
static inline int receive_samples_after_sync_from_hyper_rx_frame_type_two(srslte_ue_sync_t *q) {
  ...
  if(q->num_of_samples_still_in_buffer >= q->frame_len) {
    UE_SYNC_DEBUG("receive_samples_after_sync_from_hyper_rx_frame_type_two() - subframe_buffer_counter: %d - num_of_samples_still_in_buffer: %d - frame_len: %d - sf_len: %d\n",q->subframe_buffer_counter,q->num_of_samples_still_in_buffer,q->frame_len,q->sf_len);
    UE_SYNC_DEBUG("peak_idx: %d - peak_value: %f - nof_subframes_to_rx: %d\n",q->peak_idx, q->sfind.peak_value, q->sfind.nof_subframes_to_rx);
  }
  ...
  if(q->num_of_samples_still_in_buffer < q->frame_len) {
    ...
    if(q->recv_callback_hyper_rx(q->stream, (q->input_buffer[q->subframe_buffer_counter]+q->frame_len+q->num_of_samples_still_in_buffer), (q->sf_len-q->num_of_samples_still_in_buffer)) < 0) {
      UE_SYNC_ERROR("Error receiving samples at receive_samples_after_sync_from_hyper_rx_frame_type_two() - subframe_buffer_counter: %d - num_of_samples_still_in_buffer: %d - num_of_samples_still_in_buffer: %d\n",q->subframe_buffer_counter,q->num_of_samples_still_in_buffer,q->num_of_samples_still_in_buffer);
      return SRSLTE_ERROR;
    }
    // Reset counters.
    q->num_of_samples_still_in_buffer = 0;
...
// Returns 1 if the subframe is synchronized in time, 0 otherwise.
int srslte_ue_sync_hyper_rx_frame_type_two(srslte_ue_sync_t *q) {
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  ...
  if(q != NULL) {
    ...
    switch(q->state) {
      case SF_FIND:
      {
        ...
        if(receive_samples_from_hyper_rx_frame_type_two(q)) {
          UE_SYNC_ERROR("Error receiving samples at srslte_ue_synchronize().\n",0);
          return SRSLTE_ERROR;
        }
        ...
        int sync_ret = srslte_sync_find_hyper_rx_frame_type_two(&q->sfind, q->input_buffer[q->subframe_buffer_counter], 0, &q->peak_idx);
        if(sync_ret != SRSLTE_SYNC_FOUND) {
          sync_ret = srslte_sync_find_hyper_rx_frame_type_two(&q->sfind, q->input_buffer[q->subframe_buffer_counter], (q->sf_len/2), &q->peak_idx);
        }
        switch(sync_ret) {
          case SRSLTE_SYNC_ERROR:
            ret = SRSLTE_ERROR;
            ...
            break;
          case SRSLTE_SYNC_FOUND:
            ret = find_peak_ok_hyper_rx_frame_type_two(q);
            break;
          case SRSLTE_SYNC_FOUND_NOSPACE:
            // If a peak was found but there is not enough space for SSS/CP detection, rearrange a few samples.
            srslte_sync_reset(&q->sfind);
      ...
      case SF_TRACK:
      {
        ...
        if(receive_samples_after_sync_from_hyper_rx_frame_type_two(q)) {
...
// ######################## Frame struture II (END) ############################

// ####################### Frame struture I (BEGIN) ############################
static inline int receive_samples_from_hyper_rx_frame_type_one(srslte_ue_sync_t *q) {
...
static inline int receive_samples_after_sync_from_hyper_rx_frame_type_one(srslte_ue_sync_t *q) {
  ...
  // Get N subframes from the USRP getting more samples and keeping the previous samples, if any
  if(q->recv_callback_hyper_rx(q->stream, (q->input_buffer[q->subframe_buffer_counter]+q->frame_len+q->num_of_samples_still_in_buffer), (2*q->sf_len-q->num_of_samples_still_in_buffer)) < 0) {
    return SRSLTE_ERROR;
  }
...
static inline int find_peak_ok_hyper_rx_frame_type_one(srslte_ue_sync_t *q) {
  ...
  if(q->frame_find_cnt >= q->nof_avg_find_frames || (q->peak_idx-q->frame_len) < 2*q->fft_size) {
    ...
    if(num_of_remaining_samples == 0) {
      ...
      memcpy((uint8_t*)q->input_buffer[q->subframe_buffer_counter], (uint8_t*)(q->input_buffer[q->subframe_buffer_counter]+offset), num_of_samples_to_copy*sizeof(cf_t));
      // We read samples if there still are samples to be read from USRP in order to complete the subframe IQ samples.
      if(q->recv_callback_hyper_rx(q->stream, (q->input_buffer[q->subframe_buffer_counter]+num_of_samples_to_copy), num_of_remaining_samples) < 0) {
        return SRSLTE_ERROR;
      }
    }
...
// Returns 1 if the subframe is synchronized in time, 0 otherwise.
// Uses samples coming from Hypervisor Rx.
int srslte_ue_sync_hyper_rx_frame_type_one(srslte_ue_sync_t *q) {
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  ...
  if(q != NULL) {
    ...
    if(!q->is_find_peak_ok) {
      if(receive_samples_from_hyper_rx_frame_type_one(q)) {
        UE_SYNC_ERROR("Error receiving samples.\n",0);
        return SRSLTE_ERROR;
      }
    } else {
      q->is_find_peak_ok = false;
      if(receive_samples_after_sync_from_hyper_rx_frame_type_one(q)) {
        UE_SYNC_ERROR("Error receiving samples after sync.\n",0);
        return SRSLTE_ERROR;
      }
    }
    ...
    switch(q->state) {
      case SF_FIND:
      {
        //struct timespec start_time;
        //clock_gettime(CLOCK_REALTIME, &start_time);
        int sync_ret = srslte_sync_find_hyper_rx_frame_type_one(&q->sfind, q->input_buffer[q->subframe_buffer_counter], 0, &q->peak_idx);
        if(sync_ret != SRSLTE_SYNC_FOUND) {
          sync_ret = srslte_sync_find_hyper_rx_frame_type_one(&q->sfind, q->input_buffer[q->subframe_buffer_counter], (q->sf_len/2), &q->peak_idx);
        }
        //if(sync_ret == SRSLTE_SYNC_FOUND)
        //  PHY_PROFILLING_AVG3("Avg. find peak time: %f - min: %f - max: %f - max counter %d - diff >= 1ms: %d - total counter: %d - perc: %f\n",helpers_profiling_diff_time(start_time), 1.0, 10000);
        switch(sync_ret) {
          case SRSLTE_SYNC_ERROR:
            ret = SRSLTE_ERROR;
            UE_SYNC_ERROR("vPHY Rx ID: %d - Error finding correlation peak: (%d)\n", q->vphy_id, ret);
            return SRSLTE_ERROR;
          case SRSLTE_SYNC_FOUND:
            ret = find_peak_ok_hyper_rx_frame_type_one(q);
            break;
          case SRSLTE_SYNC_FOUND_NOSPACE:
            // If a peak was found but there is not enough space for SSS/CP detection, rearrange a few samples.
            UE_SYNC_ERROR("vPHY Rx ID: %d - No space for SSS/CP detection.\n", q->vphy_id, 0);
            srslte_sync_reset(&q->sfind);
            ret = SRSLTE_SUCCESS;
            break;
          default:
        ...
        if(q->do_agc) {
          srslte_agc_process(&q->agc, (q->input_buffer[q->subframe_buffer_counter] + q->subframe_start_index), q->sf_len);
        }
        ...
        if(sync_ret == SRSLTE_SYNC_FOUND && ret == 1 && srslte_sync_get_cfo_correction_type(&q->sfind) == CFO_CORRECTION_CP) {
          srslte_sync_set_cfo_new(&q->sfind, srslte_sync_cfo_estimate_cp(&q->sfind, (q->input_buffer[q->subframe_buffer_counter] + q->subframe_start_index)));
          //UE_SYNC_PRINT("vPHY ID: %d - Estimated CFO: %1.4f [Hz]\n", q->vphy_id, srslte_sync_get_cfo_new(&q->sfind)*15000.0);
        }
        ...
        if(q->correct_cfo && sync_ret == SRSLTE_SYNC_FOUND && ret == 1) {
          ...
          if(fabs(cfo_estimate) > 0.01) { // Only correct if CFO is greater than 150 Hz (i.e., 0.01).
            ...
            srslte_cfo_correct_finer(&q->sfind.cfocorr_finer,
                          ...
                          (q->input_buffer[q->subframe_buffer_counter] + q->subframe_start_index),
                          -cfo_estimate / q->fft_size);
            //UE_SYNC_PRINT("vPHY Rx ID: %d - FFT size: %d - CFO before: %1.4f - CFO after: %1.4f\n", q->vphy_id, q->fft_size, (cfo_estimate*15000.0), (15000.0*srslte_sync_cfo_estimate_cp(&q->sfind, (q->input_buffer[q->subframe_buffer_counter] + q->subframe_start_index))));
#else
            srslte_cfo_correct(&q->sfind.cfocorr,
                          (q->input_buffer[q->subframe_buffer_counter] + q->subframe_start_index),
                          (q->input_buffer[q->subframe_buffer_counter] + q->subframe_start_index),
...
// ######################## Frame structure I (END) ############################

// ################ Generic functions for both frame structures ################
int srslte_ue_sync_init_with_hyper_rx(srslte_ue_sync_t *q,
                                      srslte_cell_t cell,
                                      int (recv_callback_hyper_rx)(void*, void*, uint32_t),
                                      void *stream_handler,
                                      int initial_subframe_index,
                                      bool enable_cfo_correction,
                                      bool decode_pdcch,
                                      ...
                                      float pss_peak_threshold)
{
  ...
  if(q                                    != NULL &&
     stream_handler                       != NULL &&
     srslte_nofprb_isvalid(cell.nof_prb)  &&
     recv_callback_hyper_rx               != NULL)
  {
    ret = SRSLTE_ERROR;

    bzero(q, sizeof(srslte_ue_sync_t));

    UE_SYNC_DEBUG("vPHY Rx ID: %d - CFO Correction: %s (%d)\n", vphy_id, enable_cfo_correction==true ? "Enabled":"Disabled", enable_cfo_correction);
    ...
    q->subframe_counter = 0; // This counter will be used to count the number of subframes after a peak is detected and first subframe is correctly decoded.
    q->stream = stream_handler;
    q->recv_callback_hyper_rx = recv_callback_hyper_rx;
    q->cell = cell;
    q->fft_size = srslte_symbol_sz(q->cell.nof_prb);
    q->sf_len = SRSLTE_SF_LEN(q->fft_size);
    q->file_mode = false;
    ...
    if(srslte_sync_init_vphy(&q->sfind, q->frame_len, q->frame_len, q->fft_size, cell.id, q->initial_subframe_index, decode_pdcch, node_id, q->vphy_id, phy_filtering, q->vphy_id_to_plot)) {
      fprintf(stderr, "Error initiating sync find\n");
      goto clean_exit;
    }
    if(cell.id == 1000) {
      if(srslte_sync_init_vphy(&q->strack, q->frame_len, TRACK_FRAME_SIZE, q->fft_size, cell.id, q->initial_subframe_index, decode_pdcch, node_id, q->vphy_id, phy_filtering, q->vphy_id_to_plot)) {
        fprintf(stderr, "Error initiating sync track\n");
        goto clean_exit;
      }
    } else {
      if(srslte_sync_init_vphy(&q->strack, q->frame_len, SRSLTE_CP_LEN_NORM(1,q->fft_size), q->fft_size, cell.id, q->initial_subframe_index, decode_pdcch, node_id, q->vphy_id, phy_filtering, q->vphy_id_to_plot)) {
        fprintf(stderr, "Error initiating sync track.\n");
        goto clean_exit;
      }
    }
    ...
    if(cell.id == 1000) {
      ...
      q->nof_avg_find_frames = FIND_NOF_AVG_FRAMES;
      srslte_sync_set_threshold(&q->sfind, pss_peak_threshold);
      srslte_sync_set_threshold(&q->strack, 1.2);
...
// Retrieve synchronized subframe with samples coming from Hypervisor Rx.
int srslte_ue_sync_get_subframe_with_hyper_rx(srslte_ue_sync_t *q) {
  int ret = -1;

  // Try to synchronize to a subframe.
  ret = srslte_ue_sync_hyper_rx_frame_type_two(q);
  // If one of the synchronizer functions returns 1.
  if(ret == 1) {
    // Update the previous counter value so that it can be used the read the last synchronized subframe.
    q->previous_subframe_buffer_counter_value = q->subframe_buffer_counter;
    // Increase the subframe buffer counter.
