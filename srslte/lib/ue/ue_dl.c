...
int srslte_ue_dl_init(srslte_ue_dl_t *q, srslte_cell_t cell) {
  return srslte_ue_dl_init_generic(q, cell, 0);
}

int srslte_ue_dl_init_generic(srslte_ue_dl_t *q, srslte_cell_t cell, uint32_t vphy_id) {
      ...
      srslte_cell_isvalid(&cell))
  {
    ...
    if (srslte_pdsch_init_generic(&q->pdsch, q->cell, q->vphy_id)) {
    ...
    q->sf_symbols = (cf_t*)srslte_vec_malloc(CURRENT_SFLEN_RE * sizeof(cf_t));
    if(!q->sf_symbols) {
      perror("malloc");
      goto clean_exit;
    }
    // Initialize memory.
    bzero(q->sf_symbols, CURRENT_SFLEN_RE * sizeof(cf_t));
    for(uint32_t i=0;i<q->cell.nof_ports;i++) {
      q->ce[i] = (cf_t*)srslte_vec_malloc(CURRENT_SFLEN_RE * sizeof(cf_t));
      if(!q->ce[i]) {
        perror("malloc");
        goto clean_exit;
      }
      // Initialize memory.
      bzero(q->ce[i], CURRENT_SFLEN_RE * sizeof(cf_t));
    }
...
int srslte_ue_dl_decode2(srslte_ue_dl_t *q, cf_t *input, uint8_t *data, uint32_t tti, uint32_t mcs) {
  return srslte_ue_dl_decode_rnti2(q, input, data, tti, q->current_rnti, mcs);
}

int srslte_ue_dl_decode_rnti2(srslte_ue_dl_t *q, cf_t *input, uint8_t *data, uint32_t tti, uint16_t rnti, int mcs) {
  float cfi_corr;
