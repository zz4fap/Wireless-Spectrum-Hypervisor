...
int srslte_interp_linear_vector_init(srslte_interp_linsrslte_vec_t *q, uint32_t vector_len)
{
  ...
  if (q) {
    ...
    ret = SRSLTE_SUCCESS;
    q->diff_vec = (cf_t*)srslte_vec_malloc(vector_len * sizeof(cf_t));
    if (!q->diff_vec) {
      perror("malloc");
      return SRSLTE_ERROR;
    }
    // Initialize memory.
    bzero(q->diff_vec, vector_len * sizeof(cf_t));
...
int srslte_interp_linear_init(srslte_interp_lin_t *q, uint32_t vector_len, uint32_t M)
{
  ...
  if (q) {
    ...
    bzero(q->diff_vec, vector_len * sizeof(cf_t));
    q->diff_vec2 = (cf_t*)srslte_vec_malloc(M * vector_len * sizeof(cf_t));
    if (!q->diff_vec2) {
      perror("malloc");
      free(q->diff_vec);
      return SRSLTE_ERROR;
    }
    bzero(q->diff_vec2, M * vector_len * sizeof(cf_t));
    q->ramp = (float*)srslte_vec_malloc(M * sizeof(float));
    if (!q->ramp) {
      perror("malloc");
      free(q->ramp);
      free(q->diff_vec);
