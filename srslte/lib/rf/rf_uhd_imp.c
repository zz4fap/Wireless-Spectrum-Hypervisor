...
#define MEASURE_TX_TIME_DIFF 0 // Measure time difference betwwen timestamp and current time if set to 1.
...
int rf_uhd_open(char *args, void **h) {
  if(h) {
    ...
    *h = handler;
    ...
    // Create streams for channels 0 and 1 if ENABLE_HW_RF_MONITOR equal to 0, otherwise only one stream is created.
  ...
  if(blocking) {
    ...
    do {
      ...
      if(is_start_of_burst && n == 0) {
        ...
        RF_UHD_PRINT("PHY ID: %d - Time difference: %" PRIu64 "\n", channel, (tx_timestamp-fpga_time_now));
      ...
      //RF_UHD_PRINT("tx_samples: %d - is_start_of_burst: %d - is_end_of_burst: %d - Transfer time: %f\n",tx_samples,is_start_of_burst,is_end_of_burst,helpers_profiling_diff_time(transfer_timespec));
