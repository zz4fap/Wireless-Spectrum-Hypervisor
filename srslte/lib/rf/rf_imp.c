...
void rf_channel_emulator_add_awgn(srslte_rf_t *rf, float noise_floor, float SNRdB) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->chan.add_awgn_ptr(((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator, noise_floor, SNRdB);
}

void rf_channel_emulator_add_carrier_offset(srslte_rf_t *rf, float dphi, float phi) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->chan.add_carrier_offset_ptr(((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator, dphi, phi);
}

void rf_channel_emulator_add_multipath(srslte_rf_t *rf, cf_t *hc, unsigned int hc_len) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->chan.add_multipath_ptr(((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator, hc, hc_len);
}

void rf_channel_emulator_add_shadowing(srslte_rf_t *rf, float sigma, float fd) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->chan.add_shadowing_ptr(((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator, sigma, fd);
}

void rf_channel_emulator_print_channel(srslte_rf_t *rf) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->chan.print_channel_ptr(((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator);
}

void rf_channel_emulator_estimate_psd(srslte_rf_t *rf, cf_t *sig, uint32_t nof_samples, unsigned int nfft, float *psd) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->chan.estimate_psd_ptr(sig, nof_samples, nfft, psd);
}

void rf_channel_emulator_create_psd_script(srslte_rf_t *rf, unsigned int nfft, float *psd) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->chan.create_psd_script(nfft, psd);
}

void rf_channel_emulator_set_channel_impairments(srslte_rf_t *rf, bool flag) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->set_channel_impairments(((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator, flag);
}

void rf_channel_emulator_set_cfo_freq(srslte_rf_t *rf, float cfo_freq) {
  ((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator->chan.set_cfo_freq(((rf_ch_emulator_handler_t*) rf->handler)->ch_emulator, cfo_freq);
}
