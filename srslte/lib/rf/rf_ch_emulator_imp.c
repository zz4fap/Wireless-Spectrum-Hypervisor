...
int rf_ch_emulator_open(char *args, void **h) {
  if(h) {
    ...
    *h = handler;
    ...
    CH_EMULATOR_INFO("Initializing Channel Emulator...\n",0);

    int ret = channel_emulator_initialization(handler->ch_emulator);

    CH_EMULATOR_INFO("Channel Emulator initialized.\n",0);
...
double rf_ch_emulator_set_tx_srate(void *h, double freq, size_t channel) {
  ...
  RF_CH_EMULATOR_PRINT("Subframe length set to: %d\n", ret);
  if(ret <= 0) {
    return -1.0;
  }
  return freq;
...
int rf_ch_emulator_recv_with_time(void *h,
                    ...
                    size_t channel) {
  rf_ch_emulator_handler_t *handler = (rf_ch_emulator_handler_t*) h;
  int ret = channel_emulator_recv(handler->ch_emulator, data, nof_samples, blocking);
  return ret;
}
...
int rf_ch_emulator_send_timed(void *h,
                     ...
                     size_t channel) {
  rf_ch_emulator_handler_t* handler = (rf_ch_emulator_handler_t*) h;
  int ret = channel_emulator_send(handler->ch_emulator, data, nof_samples, blocking, is_start_of_burst, is_end_of_burst);
  return ret;
}
