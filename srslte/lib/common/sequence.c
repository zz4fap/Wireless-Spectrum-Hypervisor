...
int srslte_sequence_init(srslte_sequence_t *q, uint32_t len) {
  ...
  if(!q->c) {
    q->c = (uint8_t*)srslte_vec_malloc(len * sizeof(uint8_t));
    if(!q->c) {
      return SRSLTE_ERROR;
    }
    bzero(q->c, len * sizeof(uint8_t));
    q->c_bytes = (uint8_t*)srslte_vec_malloc(len * sizeof(uint8_t)/8+8);
    if(!q->c_bytes) {
      return SRSLTE_ERROR;
    }
    bzero(q->c_bytes, len * sizeof(uint8_t)/8+8);
    q->c_float = (float*)srslte_vec_malloc(len * sizeof(float));
    if(!q->c_float) {
      return SRSLTE_ERROR;
    }
    bzero(q->c_float, len * sizeof(float));
    q->c_short = (short*)srslte_vec_malloc(len * sizeof(short));
    if(!q->c_short) {
      return SRSLTE_ERROR;
    }
    bzero(q->c_short, len * sizeof(short));
    q->len = len;
  }
  return SRSLTE_SUCCESS;
}
