...
int main(int argc, char *argv[]) {
  ...
  uint32_t channel = 0, nof_tx_slots = 0;
  float milliseconds_in_the_future = 1.0;
  int numOfBytes = 3*85, slots_to_tx = 1; // Number of bytes for 5 MHz and MCS 0.
  // Create some data.
  ...
  while(!go_exit && nof_tx_slots < MAX_TX_SLOTS) {
    ...
    basic_ctrl.length = slots_to_tx*85;

    basic_ctrl.ch = channel;

    // Add some time to the current time.
    basic_ctrl.timestamp = basic_ctrl.timestamp + (uint64_t)(milliseconds_in_the_future*1000.0 + 400.0); // Line for timed commands.
    //basic_ctrl.timestamp = basic_ctrl.timestamp + (uint64_t)(milliseconds_in_the_future*1000.0 + 1000.0); // Line for wait for burst ACK.

    //printf("Send at timestamp: %" PRIu64 " - channel %d - TX slots: %d\n", basic_ctrl.timestamp, basic_ctrl.ch, slots_to_tx);

    communicator_send_basic_control(handle, &basic_ctrl);

    channel = rand() % 6;

    milliseconds_in_the_future = slots_to_tx;
    slots_to_tx = ((rand() % 3) + 1);
