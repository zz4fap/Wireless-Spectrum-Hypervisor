...
void rf_monitor_set_handler(srslte_rf_t *rf, transceiver_args_t *prog_args) {
  ...
  rf_monitor_handle.central_frequency = prog_args->radio_center_frequency; // central frequency at which the sensing module should operate.
  ...
  rf_monitor_handle.vphy_sampling_rate = helpers_get_bw_from_nprb(prog_args->nof_prb); // sampling rate used by the vPHY.
  rf_monitor_handle.radio_sampling_rate = helpers_get_bw_from_nprb(prog_args->radio_nof_prb); // sampling rate used by the radio (i.e., USRP)
...
int rf_monitor_initialize(transceiver_args_t *prog_args) {
  ...
  // Retrieve RF object.
  srslte_rf_t *rf = hypervisor_ctrl_get_rf_object();

  // Check the number of channels.
  if(rf->num_of_channels <= 1) {
    RF_MONITOR_ERROR("Number of channels must be greater than 1.\n",0);
    return -1;
  }
  ...
  float rx_freq = helpers_calculate_vphy_channel_center_frequency(rf_monitor_handle.central_frequency, rf_monitor_handle.radio_sampling_rate, rf_monitor_handle.vphy_sampling_rate, rf_monitor_handle.lbt_channel_to_monitor);
  ...
  RF_MONITOR_INFO("Frequency to monitor set to: %.2f [MHz]\n", rx_freq/1000000.0);
...
int rf_monitor_uninitialize() {
  int status;
  int rc;
  // If number of channels is not greater than 1 then just leave as there was no initialization.
  if((rf_monitor_handle.rf)->num_of_channels <= 1) {
    return 0;
  }
...
void rf_monitor_print_handle() {
  ...
  RF_MONITOR_PRINT("vphy_sampling_rate: %1.2f [MHz]\n",rf_monitor_handle.vphy_sampling_rate/1000000.0);
...
void rf_monitor_change_channel_to_monitor(uint32_t lbt_channel_to_monitor) {
  ...
  double tx_channel_center_freq = helpers_calculate_vphy_channel_center_frequency(rf_monitor_handle.central_frequency, rf_monitor_handle.radio_sampling_rate, rf_monitor_handle.vphy_sampling_rate, lbt_channel_to_monitor);
