...
//#define FIR_Coeffs_14 {0, 0, 0, 0, 0, 0, -1, 0, 3, -2, -4, 7, 2, -12, 4, 15, -15, -13, 28, 1, -38, 18, 38, -45, -24, 70, -7, -82, 54, 72, -104, -31, 141, -37, -149, 123, 111, -205, -23, 254, -106, -243, 253, 152, -378, 20, 435, -254, -381, 504, 187, -707, 152, 784, -615, -650, 1149, 210, -1685, 690, 2143, -2559, -2451, 10110, 18944}
//#define FIR_Coeffs_3 {0 ,0 ,0 ,0 ,1 ,0 ,-1 ,3 ,-2 ,-2 ,6 ,-7 ,1 ,9 ,-15 ,9 ,8 ,-24 ,24 ,-1 ,-30 ,43 ,-22 ,-25 ,62 ,-54 ,-4 ,72 ,-93 ,39 ,61 ,-129 ,100 ,19 ,-145 ,172 ,-58 ,-126 ,237 ,-168 ,-54 ,270 ,-297 ,78 ,245 ,-419 ,271 ,132 ,-498 ,512 ,-95 ,-489 ,778 ,-464 ,-327 ,1037 ,-1035 ,106 ,1255 ,-2018 ,1215 ,1400 ,-5044 ,8217 ,23296}
//#define FIR_Coeffs_5 {0, 0, 0, 0, 0, 1, -2, 1, 0, -4, 6, -7, 3, 4, -13, 18, -15, 3, 16, -32, 37, -24, -5, 39, -63, 61, -28, -27, 81, -107, 86, -19, -71, 144, -162, 105, 13, -147, 234, -225, 107, 83, -267, 355, -290, 75, 214, -454, 521, -351, -21, 458, -767, 772, -400, -265, 987, -1438, 1309, -432, -1140, 3123, -5064, 6479, 25771}
#define FIR_Coeffs_14 {0 ,0 ,0 ,0 ,-1 ,1 ,0 ,-3 ,3 ,1 ,-6 ,5 ,4 ,-13 ,9 ,9 ,-22 ,12 ,16 ,-34 ,15 ,28 ,-49 ,16 ,45 ,-68 ,15 ,68 ,-90 ,11 ,98 ,-114 ,0 ,137 ,-141 ,-18 ,186 ,-169 ,-46 ,246 ,-197 ,-88 ,322 ,-226 ,-148 ,417 ,-253 ,-233 ,539 ,-278 ,-358 ,705 ,-299 ,-551 ,951 ,-317 ,-880 ,1376 ,-330 ,-1584 ,2381 ,-339 ,-4324 ,9192 ,21504}
#define FIR_Coeffs_3 {0 ,0 ,0 ,0 ,0 ,1 ,-2 ,2 ,0 ,-4 ,6 ,-6 ,0 ,9 ,-15 ,13 ,0 ,-18 ,30 ,-24 ,0 ,31 ,-50 ,40 ,0 ,-50 ,79 ,-62 ,0 ,76 ,-118 ,92 ,0 ,-110 ,169 ,-130 ,0 ,154 ,-237 ,182 ,0 ,-214 ,328 ,-251 ,0 ,296 ,-455 ,350 ,0 ,-416 ,645 ,-501 ,0 ,614 ,-970 ,773 ,0 ,-1017 ,1694 ,-1449 ,0 ,2443 ,-5200 ,7370 ,24576}
#define FIR_Coeffs_5 {0 ,0 ,0 ,0 ,0 ,0 ,-1 ,2 ,-4 ,4 ,-2 ,-2 ,7 ,-12 ,14 ,-10 ,0 ,14 ,-27 ,33 ,-28 ,9 ,19 ,-47 ,64 ,-59 ,30 ,17 ,-69 ,105 ,-109 ,72 ,0 ,-86 ,156 ,-181 ,142 ,-43 ,-91 ,214 ,-279 ,252 ,-126 ,-69 ,273 ,-411 ,420 ,-275 ,0 ,327 ,-596 ,695 ,-553 ,169 ,371 ,-908 ,1244 ,-1196 ,648 ,400 ,-1823 ,3388 ,-4804 ,5791 ,26624}

#define BW14 1.92e6
#define BW3 3.84e6
#define BW5 5.76e6
#define bw_case(bw) ((std::abs(bw - BW14) > std::abs(bw - BW3))?((std::abs(bw - BW3) > std::abs(bw - BW5))?(5):(3)):((std::abs(bw - BW14) > std::abs(bw - BW5))?(5):(1)))
...
uhd_error uhd_usrp_get_rf_mon_stream( // RFNoC updated
    ...
    UHD_SAFE_C(
        ...
        stream_args.args["spp"] = boost::lexical_cast<std::string>(fft_size);
        stream_args.args["block_id"] = usrp.specsense_ctrls.at(0)->get_block_id().to_string();
        
        usrp.radio_ctrls.at(1)->set_arg("spp", fft_size);

        //usrp.rf_mon_streamers.push_back(
        //    usrp.rfnoc_ptr->get_rx_stream(stream_args)
        //);
        //h_s->streamer = usrp.rfnoc_ptr->get_rx_stream(stream_args);
        uhd::rx_streamer::sptr rf_mon_streamer = usrp.rfnoc_ptr->get_rx_stream(stream_args);

        //CHANGES MADE AS TIMESTAMP SHOULD NOT BE IN THE DATA VECTOR
        //std::vector<unsigned char> buff(fft_size * sizeof(std::complex<short>) + sizeof(uint64_t));
        std::vector<unsigned char> buff(fft_size * sizeof(std::complex<short>));
        uhd::rx_metadata_t md;
        //size_t num_rx_samps = 0;
        uhd::stream_cmd_t stream_cmd(uhd::stream_cmd_t::STREAM_MODE_START_CONTINUOUS);
        stream_cmd.stream_now = true;
        //usrp.rf_mon_streamers.at(0)->issue_stream_cmd(stream_cmd);
        ...
        while (true) {
            ...
            //num_rx_samps = rf_mon_streamer->recv(&buff.front(), fft_size, md);
...
uhd_error uhd_usrp_rf_monitor_initialize(
    ...
    UHD_SAFE_C(
        ...
        usrp.ddc_ctrls.at(1)->set_args(uhd::device_addr_t(str(boost::format("input_rate=184320000.0,output_rate=%lf") % rate)));
        ...
        rf_monitor_thread = new boost::thread(uhd_usrp_get_rf_mon_stream, &h, fft_size);
    )
}
...
uhd_error uhd_usrp_make( // RFNoC updated
    ...
    UHD_SAFE_C(
        ...
        P.ddc_ctrls.at(0)->set_args(uhd::device_addr_t("input_rate=184320000.0,output_rate=5760000.0"));
        ...
        P.rx_graphs.at(0)->connect(P.radio_ctrl_ids.at(0), 0, P.ddc_ctrls.at(0)->get_block_id(), uhd::rfnoc::ANY_PORT);
        ...
        uhd::rfnoc::block_id_t specsense_ctrl_id_1 = uhd::rfnoc::block_id_t(0, "SpecSense2k", 0);
        ...
        P.specsense_ctrls.push_back(P.rfnoc_ptr->get_block_ctrl<uhd::rfnoc::specsense2k_block_ctrl>(specsense_ctrl_id_1));
        ...
        P.rx_graphs.push_back(P.rfnoc_ptr->create_graph(str(boost::format("rf_monitor_graph_%d") % 1)));
        P.rx_graphs.at(1)->connect(P.radio_ctrl_ids.at(1), 0, P.ddc_ctrls.at(1)->get_block_id(), uhd::rfnoc::ANY_PORT);
        P.rx_graphs.at(1)->connect(P.ddc_ctrls.at(1)->get_block_id(), uhd::rfnoc::ANY_PORT, P.specsense_ctrls.at(0)->get_block_id(), uhd::rfnoc::ANY_PORT);
...
uhd_error uhd_usrp_get_rx_stream( // RFNoC updated
    uhd_usrp_handle h_u,
    ...
    UHD_SAFE_C(
        ...
        stream_args_args["block_id"] = usrp.ddc_ctrls.at(0)->get_block_id().to_string();
...
uhd_error uhd_usrp_get_tx_stream( // RFNoC updated
    ...
    UHD_SAFE_C(
        ...
        stream_args_args["block_port"] = str(boost::format("%d") % 0);
        uhd::stream_args_t stream_args_cpp = stream_args_c_to_cpp(stream_args);
        stream_args_cpp.args = stream_args_args;
...
uhd_error uhd_usrp_set_rx_rate( // RFNoC updated
    ...
    UHD_SAFE_C_SAVE_ERROR(h,
        ...
        get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan)->set_arg<double>("output_rate", rate); 
    )
}
...
uhd_error uhd_usrp_get_rx_rate( // RFNoC updated
    ...
    UHD_SAFE_C_SAVE_ERROR(h,
        ...
        *rate_out = get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan)->get_arg<double>("output_rate");
    )
}
...
uhd_error uhd_usrp_set_rx_freq( // RFNoC updated
    ...
    UHD_SAFE_C_SAVE_ERROR(h,
        ...
        get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan)->set_arg<double>("freq", lo_off + actual_rf_freq - rf_freq);
        double actual_dsp_freq = get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan)->get_arg<double>("freq");
...
uhd_error uhd_usrp_get_rx_freq( // RFNoC updated
    ...
    UHD_SAFE_C_SAVE_ERROR(h,
        ...
        *freq_out = get_usrp_ptrs()[h->usrp_index].radio_ctrls.at(chan)->get_rx_frequency(0) - get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan)->get_arg<double>("freq");
    )
}
...
uhd_error uhd_usrp_set_rx_channel_freq( // Does not change RF center frequency, only changes DDC frequency to switch channels within a competition band
    ...
    UHD_SAFE_C_SAVE_ERROR(h,
        ...
        //get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan+1)->set_arg<double>("freq", dsp_freq);
        get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan)->set_arg<double>("freq", radio_rx_freq[chan] - freq);
        //double actual_dsp_freq = get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan+1)->get_arg<double>("freq");
...
uhd_error uhd_usrp_get_rx_channel_freq(
    ...
    UHD_SAFE_C_SAVE_ERROR(h,
        ...
        *freq = radio_rx_freq[chan] - get_usrp_ptrs()[h->usrp_index].ddc_ctrls.at(chan)->get_arg<double>("freq");
...
uhd_error uhd_usrp_set_tx_rate( // RFNoC updated
    ...
    UHD_SAFE_C_SAVE_ERROR(h,
        ...
        int actual_bw = bw_case(rate);
        switch (actual_bw) {
            case 1:
                get_usrp_ptrs()[h->usrp_index].fir_ctrls.at(chan)->set_taps(FIR_Coeffs_14);
                break;
            case 3:
                get_usrp_ptrs()[h->usrp_index].fir_ctrls.at(chan)->set_taps(FIR_Coeffs_3);
                break;
            case 5:
                get_usrp_ptrs()[h->usrp_index].fir_ctrls.at(chan)->set_taps(FIR_Coeffs_5);
                break;
        }
    )
}
